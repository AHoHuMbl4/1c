...
> 🚀 **Настройка одним файлом: [`odata-setup/`](odata-setup/) (`setup-1c-odata.exe`)** — делает IIS,
> публикацию, OData, права, бэкап и проверку; руками остаётся только пользователь-читатель.
>
> **Пошагово вручную (установка платформы → база → пользователи → IIS → публикация OData →

    ...
    internal static class PacketSteps
    {
        ...
        public static int Run(Opts o, BaseRef bref, string odataUrl, Platform plat)
        {
            ...
            if (!o.Unattended && !Console.IsInputRedirected && !Ctx.DryRun)
            {
                ...
                Console.WriteLine("         4) права ТОЛЬКО на чтение (без них пользователь входит, но данных не видит):");
                Console.WriteLine("            карточка пользователя → вкладка «Права доступа» → «Включить в группу».");
                Console.WriteLine("            Нет группы чтения — создайте: «Группы доступа» → Создать → Профиль");
                Console.WriteLine("            «Только просмотр» → в таблицу «Пользователи» добавьте «" + probeUser + "».");
                Console.WriteLine("            Нет и профиля (Управление торговлей, Комплексная автоматизация — там его");
                Console.WriteLine("            нет из коробки): «Профили групп доступа» → Создать «Только просмотр» →");
                Console.WriteLine("            отметьте роли поиском по словам: Базовые права, Запуск, Использование,");
                Console.WriteLine("            Отчеты, Подсистема, Просмотр, Раздел, Чтение (выделить найденные →");
                Console.WriteLine("            «Включить выделенные роли») → затем группу, как выше;");

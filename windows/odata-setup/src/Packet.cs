    ...
    internal static class PacketSteps
    {
        ...
        public static int Run(Opts o, BaseRef bref, string odataUrl, Platform plat)
        {
            ...
            if (!o.Unattended && !Console.IsInputRedirected && !Ctx.DryRun)
            {
                ...
                Console.WriteLine("            и на вкладке «Права доступа» включите профиль «Только просмотр» —");
                Console.WriteLine("            без профиля пользователь входит, но данные читать НЕ может;");
                Console.WriteLine("            ОБЯЗАТЕЛЬНО включите галку «Вход в приложение разрешён» — без неё 1С");
                Console.WriteLine("            не создаёт пользователя базы, и запись остаётся только в справочнике;");

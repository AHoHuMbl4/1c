...
# 🔴 `work/hooks/` — тоже пропуск, и это найдено ПЕРЕПРОВЕРКОЙ, а не додумано: сам этот
# файл содержит образцы `SELECT … FROM`, `MERGE INTO` и прочие в списке признаков, поэтому
# без пропуска гейт спотыкался о собственный исходник (проба: коммит `7974bd3` — шесть
# признаков в `check-sql-docs.sh`). Хуки — не работа с базой, доки SereneDB им ни при чём.
SKIP_PREFIX = (".claude/", "docs/", "memory_bank/", "work/hooks/")

...
🔴 **Не выкачено на момент записи**: исполняемый код лежит в `/opt/1c-mcp-reports`, правка —
в репозитории. Выкатка отдельным шагом, с проверкой, что исполняется именно она.
...
| 4 | **0 ₽** | 3 125 757,80 | регистр выручки, величина `НДСРегл` | **величины НЕТ в сущности** |

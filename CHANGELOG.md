...
`[замер]` **Приёмочный набор — 5/8** на боевом скорере первой базы (`lm_dirichlet`),
средняя 32,6 с. Прогонялся только он: шесть скореров — это A/B-опыт, а не регрессия, и он
оставил бы базу на `dfi`, который `techContext` называет заведомо дефектным. Все три
провала — **выбор сущности** («Сколько всего мы продали?» — сущность не выбрана вовсе;
англоязычный тот же вопрос — взят регистр `значенияемрц`; «Что покупало ООО Ромашка?» —
регистр `состоянияконтрагентов`), счёт не ошибся нигде.

🔴 **Правки захода к провалам не причастны, и это проверено:** за прогон новая проверка
величины не сработала **ни разу** — ноль строк `не названа цифрами` и ни одной строки
`GATE` в журнале сервиса. И 🔴 **базой сравнения этот прогон быть не может**: против
исторических 8/8 сменились три фактора, ни один не наш — скорер `dfi`→`lm_dirichlet`,
перевекторизация первой базы в другое пространство, и мёртвый реранкер (401 на каждом
вопросе; по замеру 02.08 он стоит 14 %→36 % именно на выборе сущности).

...
это работа параллельной сессии. На `ut_test` раздел применён и замерен напрямую, полный
такт по боевой базе пойдёт следующей сборкой: в момент правки там шла приёмка 44 вопросов
соседней сессии, и подменять под ней корпус было бы порчей чужого замера.

...
## 08.08: веб-фронт — брендинг и доступы; направление фич (заготовки владельца) `[замер]` `[решение]`
...
требование лицензии при 50+ пользователях, оставлен сознательно.
...
(расписание — штатные Automations); на бэк записаны новые базы и LLM-роутер
(ложится на агентов OpenClaw, `/v1` уже маршрутизирует по `agentId`).

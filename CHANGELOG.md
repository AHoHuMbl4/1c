## 19.08: золотой набор okna собирается ИЗ ДАННЫХ базы `[код]` `[замер]`

`[код]` `ubuntu/serenedb/ab_gold_build.py` — генератор приёмочного набора. Имён
конкретной базы в нём нет: главный оборотный регистр, денежная и штучная величины,
величина-пустышка, оси разреза, справочники, пустой день и прошлый месяц берутся
из метаданных корпуса (`search_tables`, `search_refcols`, `search_entity_alias`,
`search_corpus`, `duckdb_columns`). Чисел в наборе нет — эталон считает SQL при
каждом прогоне. Пишет `ab-gold-okna.tsv` (**15** вопросов, 9 классов из живых
дефектов `CLARIFY_JOURNAL_OKNA_2026-08-18`) и снимок «вопрос → ответ»
`docs/AB_GOLD_OKNA.md` для чтения человеком.

`[замер]` Отбор идёт СРАВНЕНИЕМ, без числовых отсечек — иначе это подгонка под okna
(находка снайпера хардкода в этом же заходе): денежный итог — наибольший итог среди
величин, у которых бывают копейки («Всего» 80 300 612,08 против «Себестоимость»
49 593 855,66, хотя дробных у себестоимости больше); штучная — самая целая из
оставшихся («Количество», 31 дробное значение из 77 527); пустышка — ровно ноль по
всей сущности («Сумма»); оси разреза — только те, что база объявила ссылками на
справочник (`search_refcols`) и у которых больше одного значения, поэтому `Recorder`
(8026 значений) отпадает сам, а остаются `ТМЦ` (1558) и `Контрагент` (173);
справочники для счёта — справочники этих же осей («Номенклатура» 2383 без папок,
«Контрагенты» 349), а не самый длинный `catalog_*` (им оказался классификатор
должностей, о котором не спрашивают); пустой день — последний разрыв в собственном
ряду дат сущности (16.08 между 14.08 и 17.08), без окна «за N дней».
Остатки: у главного регистра нет колонки `RecordType`, поэтому вопрос об остатках
закрывается `no_data` — это следствие данных, а не список слов.

`[код]` `ab_scorer.py` — читает третье поле набора (`CLS=` класс, `PICK=` клики) и
идёт **путём человека**: спросили — кликает билетом `decision_id` выбранного варианта
(`src:`/`axis:`/`measure:`/`label:`, потолок `AB_MAX_CLICKS`). Режимы строк: SQL-число,
`MODE=kind`, `MODE=fork` (вилка спрошена, обе подписи человеческие, без утечки
внутреннего имени), `MODE=pivot` (пустая величина не подаётся как «0» и не закрывается
отказом). `serene_ask.py` не трогался.

`[замер]` Оффлайн `test_ab_gold.py` — **30/30** (разбор набора, клик по билету, четыре
режима годности). Живой приёмки этим заходом нет: решение владельца — прогон и правки
кода под эти вопросы делает другая сессия. Разведка живого `:8091` (4 пробных вопроса)
показала, что «сколько продали вчера» уходит в вилку `Книга продаж` ↔
`document_реализациятмц`, счёт справочника отвечается верно (349 контрагентов,
`folders=4`), а вопрос про склад закрывается `no_data` — набор эти три случая держит.

`[код]` Доступ и приборы: `work/acceptance/okna-sql.sh` (SQL под RO-ролью),
`okna-ask-probe.sh` / `okna-ask-raw.sh` (разбор ответов живого ask),
`okna-gold-build.sh` (пересборка набора), `okna-kill-gold-run.sh`.

md5: `ab_gold_build.py` **`9cd5d351`**, `ab_scorer.py` **`0b63a20c`**,
`ab-gold-okna.tsv` **`2c92c4ba`**, `test_ab_gold.py` **`12c8fa70`**,
`docs/AB_GOLD_OKNA.md` **`2d2d8cca`**. Выкаченный `serene_ask.py` на okna —
`99266ba9` (совпадает с git, e82abb5).

## 19.08: дашборды okna — сквозной вход подтверждён, панели `No data` `[замер]` `[код]`
...
`[замер]` **Выкат на фронт сделан 19.08**: установщик прогнан с
...
okna-sales: уже есть, не трогаем` (живой дашборд не перетёрт). После выката
`json_data` datasource на фронте =
`{"database":"postgres","postgresVersion":1500,"sslmode":"disable"}`.

`[код]` Шаг 9 установщика — **приёмка панелей**: `rawSql` каждой панели
каждого дашборда гоняется через `/api/ds/query` (тот же путь, что у открытой
страницы), печатается число строк, пустая или ошибочная панель роняет
установку. Заведён потому, что probe `select version()` при этой поломке
проходил (ловушка 11).

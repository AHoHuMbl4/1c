...
## 02.08: ключевые документы приведены к аудиту — семь файлов, семь коммитов
...
- **Стадии по контракту приведены к честным** `[замер]`: 🟢 11 → **7**. Понижены пп. 5, 6,
  12, 20 — все стояли по формулировке либо по замеру, которого нет; п. 17 понижен, потому
  что конвейер стоит с 29.07. Разбор каждого — таблицей в `TARGET_STATUS.md`.
- **Число «до 160 КБ на вопрос», которым закрыт п. 19, не существует** `[замер]`:
  `grep` по всему репозиторию даёт единственное вхождение — саму эту строку. Настоящий
  замер того же пункта — **6–20 тыс. символов**, и он записан здесь же в журнале.
- **Штатный backup/restore у движка ЕСТЬ** `[замер]` — три документа утверждали обратное,
  не проверив. На нашей сборке 26.07.3 отрабатывают `EXPORT DATABASE`, `IMPORT DATABASE`
  и `COPY FROM DATABASE … TO`; контрольный опыт: `EXPORTX DATABASE` → `syntax error`, то
  есть неизвестный оператор выглядит иначе. Это ровно тот отрицательный вывод «штатного
...
- **Оценка п. 2 стоит на прогоне, который устарел на пять правок пути ответа** `[замер]`:
  последний полный прогон — 30.07 22:34 (и он сам частично недействителен: вопросы 31-58
  оборваны исчерпанием ключа), последний вообще — 31.07 ночь, пачка 1. После него в
  `ubuntu/` легло не меньше пяти коммитов, меняющих путь ответа и персону, и починен сам
  прогонщик. Ни одного замера ответов с тех пор не снято, а штатным способом его сегодня
  и не повторить: сервис ответов держится процессом вне systemd.
- **Прежние ошибочные утверждения не стёрты** `[решение]`: в каждом файле они оставлены
  ...
  четыре поправки на месте: зелёный заголовок 30.07 «ноль неверных ответов» (опровергнут
  в тот же день, отзыв тогда в журнал не попал), эталоны складов и номенклатуры, размер
  промтов 4 208 → ~8 135, граф MCP «коммитится» (файла нет).
- **Спорное решает владелец, не я** — вынесено в `work/doc-audit/QUESTIONS_TO_OWNER.md`:
  ...
  1767 CSV на 826 МБ, права на выгрузку), развилка по персоне бота.
...
- **Счёты справочников верны** `[замер]`: партнёры 164, контрагенты 155, номенклатура 252,
  ...
  > 🔴 **Поправка (02.08): два из шести эталонов оказались неверными, и это выяснилось
  > позже.** Склады: верно **17 позиций (+8 групп)**, а не 25 — сверка с живой 1С 31.07

...
search_idx` — один `UPDATE … FROM` (доки sql/statements/update#update-from-other-table).
...
`[замер]` okna до: частей с parent 82, все даты пусты **79**, своя дата 3;
сирот 0; деловых без вектора **0**; корпус 635337 / векторов 611908.
Прямой фильтр периода по `doc_date` строк крупнейшей части: 0.

Выкат и «после» — следом в этой же сессии (гейт золотого требовал строку в HEAD
до выката; первую базу не мерили).

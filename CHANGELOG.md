...
имя смотреть в Конфигуратор → Администрирование → Пользователи. Для стенда
сделан запускатель C:\1c\dist\run-ut_demo.cmd (база и админ уже в команде —
остаётся Enter на пустом пароле). Exe sha256 17f11b51 на стенде и S3 (ut+buh).

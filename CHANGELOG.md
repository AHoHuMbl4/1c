...
Выкат и живой POST /ask в этом заходе не сняты: SSH-ключ деплоя закрыт
floor'ом сессии. Критерий 3 (`diag.focus` / `focus_forced` = `options[].label`
документа из того же clarify, не цифра 19) — после выката. Круга нет на
живом POST — не этот заход.

...
## 07.08: установщик — выбор базы одной клавишей + пауза на ошибке `[замер]`

По прогону владельца («после выбора базы окно закрылось и всё»): при ошибке
интерактивный запуск ждёт Enter (окно не исчезает с текстом); выбор базы —
одной цифрой без Enter (`ReadKey`, до 9 баз; пайпы и >9 баз — построчный ввод с
переспросом). `[замер]` на стенде: пайп-путь работает, сборка чистая. Попутный

...
установка на сервере (то, что работает сейчас)». Прежняя редакция называла API Alibaba
Qwen, и это давно расходилось с делом: эмбеддер Qwen3 и реранкер Qwen3-Reranker-8B стоят
в контуре владельца, `serene_ask.py` ходит к ним по `EMBED_BASE_URL` и `RERANK_URL`.

Прежняя формулировка сохранена в самом пункте одной строкой — контракт менялся, и по нему
потом читают историю решений.

⚠️ Осталось расхождение, которое я править не стал (пункт 16 владелец не называл):
строка «сейчас данные уходят в API Alibaba и DeepSeek — это временно, для тестов». После
правки пункта 5 в Alibaba не уходит ничего; наружу остаётся только модель ответов.

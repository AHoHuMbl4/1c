...
- **перезапуск юнитов** отвечал «Interactive authentication required»: выкат не мог
  закончиться рестартом сервиса, то есть замер упирался в права, а не в код. Заведён
  `/etc/sudoers.d/claudedev-units` — ровно управление юнитами `1c-*` и `daemon-reload`,
  без общего `sudo` (`visudo -c`: parsed OK).

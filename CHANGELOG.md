...
✅ **ВЫКАЧЕНО И ЗАМЕРЕНО ЖИВЬЁМ.** `deploy.sh` + перезапуск обоих юнитов, дальше — новый
прибор **`work/acceptance/step5_live.py`**: вопросы набора идут в работающий сервис, и
каждое посчитанное число пересчитывается независимо по объявленному множеству.
`[замер 04.08, вопросы 1-14]` (`runs/2026-08-04-step5-live-1-14.txt`): шаг 5 считал на **6**
вопросах, **ошибок 0** по всем пяти классам (число не сходится, выдуманное число,
невоспроизводимо, множество не объявлено, потеря молча). На восьми остальных шаг 5 не
считал — там уточнение, и это решение других шагов либо стража величины.

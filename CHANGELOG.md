## 26.08: защита плана от подгонки — механизмами, не обещаниями [решение]

[решение] Владелец 26.08: в план вписаны (1) задача **И5** — гейт «код +
приёмочный набор в одном коммите = стоп» (прецедент 993ad81; скрипт и пробы
— оркестратор в work/hooks, установка — владелец через install-gates.sh;
GOLD_SETS.md обновляется после установки, не до); (2) правило доказательства:
дельта починки обязана превышать разброс И3, иначе это шум, а не починка;
(3) постоянный принцип: правило против подгонки принимается только вместе с
его механизмом (гейт/замок) — правило на словах считается отсутствующим
(замер 03.08: напоминание исполнено 0 раз из 94).

## 26.08: PLAN_TO_TARGET — рецензия учтена [док]

[док] По рецензии плана: снято противоречие И2 (два прохода: сразу на 58 —
грубо, после И0 на client-gold — валидно); И0 — зависимость от шлюзa OData
переформулирована (шлюз штатный, поднят для всех живых баз; база без шлюзa —
мёртвый стенд); К1 — предусловие актуального контура (/health, code_md5,
скорер); Э1 — результатом считается решение по каждому заходу psql, не
число; Э5 — юнит удаляется с доказательством целостности боевого пути, а не
помечается. Новый раздел 3bis: контрольные точки ЧП1–ЧП3 с go/no-go,
сценарий «честное число хуже подогнанного», готовность к п. 11 — по
TARGET.md, численные цели — после И2/И3.

## 26.08: GPU в норме — блокировка «27B выключен» снята [решение]

[решение] Слово владельца 26.08: GPU работает, живы и эмбеддинг, и 27B, и
реранкер. Записи 25.08 «27B выключен владельцем» протухли и вводили в ложную
очередь: в `activeContext.md` исправлены «ждёт включения 27B» (очередь
разблокирована, порядок по чеклисту Ф6.5) и канон GPU; в `TEST_SECOND_BASE.md`
— примечание к входам второй базы; в `PLAN_TO_TARGET.md` — раздел 0, В2
(блокер 0 снят), порядок запуска. Живые пробы и выкаты снова разрешены.

## 26.08: п. 15 пересмотрен в части инфраструктуры [решение]

[решение] Владелец 26.08: приёмка на базе клиента (п. 15 TARGET.md) остаётся
последним шагом, но подготовка инструментов — сейчас, задача И0 в
`docs/PLAN_TO_TARGET.md`: восстановить `etalon_1c.py` (сверка эталонов с
живой 1С по OData; скрипт 31.07 утерян — в дереве отсутствует, та сверка
поймала 1 неверный эталон из 17) как постоянный закоммиченный инструмент;
генератор вопросов из `$metadata` (сущности) + `ask_journal` (формулировки);
эталон двойной (1С + корпус), расхождение → независимое прочтение агентом
без контекста кода. Проверка генератора — на ut_test/klient-1. Лаг свежести:
расхождение «данные уехали» — пометка, не FAIL. Там же С5 (прибор кандидатов
алиасов — журнал только датчик, не автопополнение; прецедент
`ask_choice_memory`).

## 26.08: схема пайплайна — первым этапом и последним [решение]

[решение] В `docs/PLAN_TO_TARGET.md` добавлены два этапа вокруг всей работы.
Этап 0 (**Ш0**) — `docs/PIPELINE.md`: одна страница со стрелками от 1С до
сообщения человеку, которую видно взглядом без чтения кода: путь данных,
путь вопроса, что едет по каждой стрелке, что запускает каждый узел и где
его код, развилки (уточнение / отказ / нет данных), места вызова языковой
модели (п. 19), условные обозначения. Этап финальный (**ШФ**) — та же схема
заново, по факту, с разделом «что изменилось против первой версии». Причина
второго прохода: к концу плана словарь наполняется тактом, флаги Ф6 стоят
в бою, часть узлов исчезает (Э5) — схема начала работы к этому моменту
начнёт врать, а устаревшая схема хуже отсутствующей.

## 26.08: план до сдачи по TARGET.md [док]

[док] `docs/PLAN_TO_TARGET.md` — что осталось до контракта, пятью потоками
(измерение / словарь / классы ответов / ворота Ф6 / эксплуатация) с явным
указанием, что идёт параллельно. Ограничение параллельности записано прямо:
`serene_ask.py` — один файл, в потоке классов работает ровно один агент,
остальные потоки правят другие файлы и идут одновременно; пересечение
проверяется по `docs/CODE_MAP_ASK.md`. Вход во весь план — независимый прогон
приёмочного набора (58 вопросов): текущие 21/25 сняты на наборе, по которому
и чинили, поэтому срок до сдачи назвать нельзя, пока нет числа на наборе,
не участвовавшем в правках. Второе неизвестное — детерминированность (п. 3):
последний замер 31.07 давал ±1 из десяти, с тех пор не повторялся.

## 25.08: полный скорер 21/25 — базлайн поднят, гейт выката PASS [замер]

[замер] Живой полный скорер по okna на кандидате `code_md5 733f693f`
(стенд `:8092`, туннель `:18092`): **21/25**, сбоев **0**, средняя **29,26 с** —
впервые выше прежнего базлайна 20/25. `check_deploy_gate.py` → **PASS**,
новых FAIL нет. Закрылся K4 («эта неделя лучше прошлой») — правкой формы
сущности. Остаток: `rank top-N` «топ-3 товара за вчера» (золото считает по
`Количество`), K5b compare-хвост, два K6. Базлайн `docs/DEPLOY_GATE.md`
переписан на 21/25, порог регресса поднят.

[замер] Ложный регресс, стоивший одного прогона: кандидат выкатывался в `/tmp`
одним файлом, а `period_relative_forms()` ищет запасной словарь рядом с модулем
(`serene_ask.py:1745`). Словаря в `/tmp` не было, в `search_meta` ключа нет
вовсе → «за прошлую неделю» не распозналось, окно молча ушло в текущую неделю
(`period_leader=wtd`), скорер показал «регресс rank». После копирования словаря
тот же вопрос даёт `prev_week`, окно `2026-08-17..2026-08-23`, эталонный лидер.
Урок — `HOW_NOT_TO §3.95`. Побочно: `code_md5` в отчёте скорера берётся из
`PROBE_CODE_PATH` (умолчание — боевой `/opt/...`), для стенда задавать явно.

## 25.08: аудит системы — 28 частей, 4 сквозных пути, итог с проверкой [док]

[док] Многоуровневая инвентаризация по коду, без догадок: L1 — 28 отчётов по
кускам (`docs/audit/parts/`, каждый факт со ссылкой путь:строка); L2 — 4
сквозных пути (`docs/audit/level2/`: вопрос, данные, выбор, эксплуатация);
L3 — сборка `docs/HOW_IT_REALLY_WORKS.md`; L4 — проверка каждого утверждения
по коду (`docs/audit/L4-verify.md`): проверено **92**, исправлено **7**,
не подтвердилось **7**. Существенное: порядок выбора меры был описан обратно
фактическому; `check-golden` числился в `git-gate.sh`, хотя это PreToolUse на
выкат. Расхождение из L2: боевой корпус пишет `build.sh`, а шаблон
`ubuntu/serenedb/systemd/1c-serene-index.service:22` всё ещё зовёт выведенный
`serene_search_build.py`.

## 25.08: словарь синонимов — обиходные слова, не морфология title [код]+[замер]

[замер] okna `search_entity_alias` (254): у сущностей только title/склонения и
имена реквизитов; `ts_lexize('search_dict_syn','клиент')` → без связи (только
«клиент»). Вопросы «сколько клиентов…» / «позиций совсем не продаётся» тянутся
вектором вопреки словарю. [код] Место: `wiki_alias.sh` запрос к модели
(~151/273) просил «SHORT NAMES… including its own title»; разведение (~401)
просило *dropping words that fit the others*. Исправлено: everyday words из
вопросов людей + title; величины только в `quantities`; collision держит общие
обиходные слова, различие — в `notEnoughFor`. Разбор
`wiki_alias_parse.filter_entity_aliases` отсекает имена/алиасы величин из
entity aliases. `search_synonym_bridge`: читатель `solr_synonyms_build.py`,
писателя в дереве нет (слот моста §7-бис) — зафиксировано в `corpus_init.sql`
и `F6_SYNONYMS_FACTS`. Живой прогон генератора — оркестратор (непустые aliases
идемпотентно не переспрашиваются). Замки: `test_wiki_alias_parse.py` **17/0**,
рядом `test_solr_synonyms_build` **28/0**, `test_build_solr_synonyms` **26/0**.

## 25.08: карта кода — зоны по якорям, не по номерам строк [код]+[замер]

[замер] После роста `serene_ask.py` 15777→15851 абсолютные `from`/`to` в
`zones.json` расходились с файлом («Покрытие зон НЕ сходится»), замок
`test_code_map.py` **14/5**. [код] Зоны задаются якорями `start`[/`end`]
(модульные функции/классы); `code_map.py` сам считает границы из AST —
сомкнутое покрытие 1..N без дыр/нахлёстов при любом росте. Исчезнувший
якорь → `AnchorError` с именем. `from_ref`/`to_ref` — справочные.
Выходы пересобраны на текущем коде (**15851** строк, **461** функций,
сквозных **29**). Замок **28/0**, в т.ч. вставка функции внутрь зоны без
правки `zones.json` и проба пропавшего якоря. `serene_ask.py` не трогали.

## 25.08: rank×sales — qty на товарной оси без названной меры [код]+[замер]

[замер] okna `/ask` «дай топ-3 товара за вчера»: окно и ранг верные, мера
`Всего` (208.38 лей) вместо эталона `Количество` (ab-gold-okna.tsv:17).
Срыв: `rank_measure_hint` → Количество, затем `sales_rank_canon_measure` →
None (`_sales_product_rank_qty`/`_rank_wants_quantity` не видят «топ»≠«top»),
call-site money-fallback перебивал на Всего.
[код] Правило: rank по оси с `target_src` товарного catalog (`sales_rank_product_axis`
через `kind_axis_hits`/resolve + `_is_product_catalog`) и мера не названа
(алиасы из данных) → `sales_qty_measure`, если есть; деньги — только когда
названы или qty нет. Money-fallback call-site не трогает товарную ось.
Замки: `test_sales_rank_canon.py` **43/0** (+класс топ-N); рядом
`test_rank_axis_anchor` **66/0**, `test_rank_leader_path` **26/0**,
`test_entity_form` **47/0**, `test_sales_canon_prefer` **74/0**.
Эталоны ab-gold не менялись.

## 25.08: машинная карта serene_ask.py [код]+[замер]

[код] Генератор `ubuntu/serenedb/code_map.py` (stdlib: ast/json/argparse) строит
карту модуля по `docs/audit/zones.json` (20 зон аудита parts 01–20, сомкнуты
без дыр/нахлёстов; конец зоны 20 = текущая длина файла **15777**). Выходы:
`docs/audit/code-map.json`, `docs/CODE_MAP_ASK.md`. `serene_ask.py` не менялся.
[замер] Оффлайн-замок `test_code_map.py` **19/0**; функций **460**, сквозных
**28**, полностью внутренних зон **0**.

## 25.08: period_relative_forms в search_meta + /health [код]+[замер]

[код] Живой дефект okna: ключа `period_relative_forms` в `search_meta` не было,
шага сборки не было — словарь держался только запасным JSON; без файла окно
«прошлая неделя» молча становилось текущей (п. 13). В такте шаг **1-period**
(`build.sh` + `period_relative_forms_load.sql`): `read_text` → `search_meta`,
копия JSON в `CSV_DIR`, вне `SKIP_BUILD`, fail-closed. В `/health` поле
`period_relative_forms: {loaded, forms}`; пустой словарь → **503** `degraded`.
Запасной путь файла — от `__file__` (cwd чужой каталог не ломает). Оффлайн
`test_period_relative_forms_ready.py` **24/0**; рядом health/rank/solr
**23+32+34+26/0**. Живой выкат такта — после слова владельца (27B выкл.).
Доки: cookbook/file_formats/read_file#read_text;
data_import_and_export/json/json_functions.

## 25.08: ASK_ENTITY_FORM — форма не лидер на развилке [код]+[замер]

[код] Координата формы F (distinct_axis / complement / compare) за флагом
`ASK_ENTITY_FORM` (умолч. 0) + гейт `entity_form_pre_entity_ok`: на шаге
pre_entity форма не отвечает, когда классов развилки >1 или форм по пулу >1 —
иначе F молчаливый лидер (п. 13). Без окна kind→catalog берётся только
stem/label SQL, не `meaning_candidates`. Оффлайн `test_entity_form.py` **40/0**.
[замер] Стейджинг okna `:8092` (md5 **52e275e5**): живая AB_PROBE **8/8**,
средняя **11,46 с**, отметка `okna probe live 0err/8`; «позиций в прайсе» —
**2384** (было 1 у молчаливого distinct). Бой `:8091` не тронут. Полный скорер
25q — следующим прогоном.

## 25.08: Ф6.5 чеклист выката + прибор замера [код]

[код] Порядок включения флагов Ф6 в бой по риску —
`docs/F6_ROLLOUT_CHECKLIST.md` (freshness → IVF → solr; `ASK_SQL_RRF` уже в
бою; `ASK_CALENDAR_AXIS` на okna сейчас no-op — нет `calendar_*` в meta).
Прибор `ubuntu/serenedb/f6_rollout_measure.sh` сам флаги не ставит.
Оффлайн-замок `test_f6_rollout_measure.py` **15/0**. Живой ДО/ПОСЛЕ — после
включения 27B.

## 25.08: dev SereneDB :7890 залип [замер]

[замер] Юнит `serenedb.service` active с 21.08, listen `:7890`, но query path
мёртв: ESTAB **337** (из них ask@ut_test **232** psql + ask@postgres **56**),
все потоки **%CPU 0**, RSS ~2,5 GiB не растёт, WAL **115 706** B застыл с
**2026-08-24T15:25Z**, `SELECT 1` — timeout. Вердикт: залип, не «занят».
Рестарт — решение владельца (ненулевой WAL → риск OOM-петли). Отчёт
`docs/DEV_ENGINE_HANG_2026-08-25.md`. Из‑за этого 9 live-замков свода не
зачтены.

## 25.08: свод оффлайн-замков — 2150 зелёных [замер]

[замер] После правок serenedb замки подтянуты к коду (мок diag/prior, sum,
аудит §10, pytest; `serene_ask.py` не трогали). Свод
`docs/LOCKS_SWEEP_2026-08-25.md`: **79** файлов замков, **2150** оффлайн-случаев,
**0** падений. Live **9** файлов — требуют живого контура (`:7890` timeout),
не зачтены.

## 25.08: §7bis — валютная ось (проект) + набор календаря в AB [код][замер][решение]

[код] Шаг 3 календаря: `ab-calendar-axis-okna.tsv` (**6** вопросов) и режим
`AB_CALENDAR_AXIS` (умолч. off) в scorer; базовый gold/probe не смешан. Замки
`test_ab_calendar_axis_set` **22/0**, `test_ab_scorer_v2_modes` **19/0**.

[замер][решение] Следующая ось надстройки — валютная (не гео): на okna валют
**6**, курсов **3824**, константа БУ **1**, на реализации FX-шапок **40**/8240,
Σ документа **78 789 043,20** vs учёта **79 164 480,23**. Проект
`work/currency-axis-design.md`; кода оси нет. Гео слабее (города **116**,
«чистых» ~**10**, ФИАС нет).

## 25.08: память выбора — замер коллизий пуст [замер][решение]

[замер] okna (`serene_ro`): `ask_choice_memory` **0** строк / **0** классов
коллизий при `ask_journal` **1445**, fork B+C **142**, `ticket_used` **104**.
Отчёт `docs/MEMORY_COLLISIONS_2026-08-25.md`.

[решение] Замер непоказателен: «нет коллизий при нуле строк» не доказывает
безопасность. Включать память в бой по этим числам нельзя.

## 25.08: Ф6.6 аудит хардкода + warehouse по Edm [код][замер]

[замер] Аудит `docs/F6_HARDCODE_AUDIT.md`: **12** находок — **1** починена
(список городов стенда в `measure_resolver.py` → argv; замок terms **4/0**),
**9** законны (словарь/календарь/`$metadata`/бюджеты), **2** оставлены
оркестратору (`serene_ask` «не существует»; balance «количество» вне диффа Ф6).
Сопутствующие замки ночи **195/0** (solr 28+14+26, calendar 30+36, ivf 18,
freshness 32, rrf 7, …).

[код] Находка №11: форма `accumulation_warehouse` в `corpus_build.sql` §1-тер —
числовой ресурс по типам Edm (как §1-кватер), не языковой `IN
quantity/количество`. Замок `test_balance_map_meta_build` **18/0**; сверка на
живой okna старое **3 ⊂** новое **13** (оба: «Количество» среди nums).

## 25.08: Ф6.3 хвост — solr_synonyms в такте build.sh [код]

[код] После `wiki_alias` каждый такт компилирует словарь из alias+bridge
(шаг **7-solr**: DROP+CREATE файлом, fail-closed; сверх лимита — стоп без
обрезки). Замки `test_build_solr_synonyms` **26/0**, `test_solr_synonyms_build`
**28/0**. Фактура такта — `docs/F6_SYNONYMS_FACTS.md`.

## 25.08: документы догнали Ф6 (поверхности / флаги / сеть) [код][замер][решение]

[код] `docs/HOW_IT_WORKS.md` и `docs/ARCHITECTURE.md` приведены к коду фазы Ф6
(`PLAN_UPGRADE_NATIVE.md` §9): честно перечислены поверхности кандидатов
(буквальный + 4 RRF; при `ASK_SQL_RRF=1` — 5-я ANN корпуса), SQL-RRF, IVF-резолвер,
query-side `solr_synonyms`, native freshness `/health`, ось календаря. У каждого
флага — умолчание `"0"` и где включено (бой `:8091` / стейджинг `:8092` / нигде).
`docs/NETWORK.md` — канон GPU vSwitch 4003: 10.3.1.11 embed:8000/rerank:8005,
10.3.1.12 27B:8000/embed:8002; whisper и diarize/pyannote остановлены; внутри сети
без фильтра, режется внешний периметр.

[замер] Ф6.1 бой: флаг 0 — 12/24, 60,17 с; флаг 1 — 12/24, 76,31 с; VERDICT_DIFFS 0
(+16 с под GPU 100 % klient-1). Ф6.2/Ф6.4 на `:8092`: AB_PROBE 8/8, `/health` 0,13 с.
Ф6.3 словарь okna: 221 правило / 23 305 байт; бой не включён.

[решение] Документы — незавершённая работа по правилам проекта; `TARGET_STATUS.md`
в этом заходе не трогали (чужой трек).
...
императив, готовая команда, ответ тремя строками-метками, «Запрещено» без пояснений.
[замер 24.08] Держат формат: haiku (ccproxy) и deepseek-v4-flash (спауны agent-82/86/87,
exit 0, инициативы ноль). Модель обёрток переключена haiku → deepseek/deepseek-v4-flash
(живой конфиг; канон отстаёт — см. ловушку про расхождение).

## 24.08: /health не сканирует витрину по сущностям [код] [замер]

[код] `GET /health` в `serene_ask.py` вызывал `_measure_health_gap` → цикл
`_vitrina_objects` / `_table_has_ref_key` по каждой таблице из `search_tables`
(4–6 `psql()` на сущность: `duckdb_tables`, `tmp3_key`/`duckdb_columns`,
`count`/`count(DISTINCT Ref_Key)` с `query_table`) плюс UNION ALL по дням
`Period`/`Date`. Это не очередь однопоточного HTTP: сервер —
`ThreadingHTTPServer`; `/ask` при залипшем `/health` отвечает (локально 401 за
52 мс). Кэш `ASK_HEALTH_GAP_TTL` процессный, ключ без времени, но пишется
**после** полного обхода и без замка — повтор сторожа запускал второй обход.
`[замер 24.08]` ut_test: 3 сущности `_vitrina_objects` = **26,1 с** / 12
`psql()`; `SELECT 1` через тот же `psql()` ~300 мс; локальный `/health` `:8091`
и `:8099` — curl 28, 0 байт; journald 200 + BrokenPipe. Перепись
`search_coverage` тем же смыслом — **0,06 с**. Дверь теперь читает перепись
одним SQL; кэш под `_health_gap_lock`. Не подгонка таймаута.

[замер] `test_health_gap` **23/23**. Живой `_measure_health_gap` на ut_test
после правки — один запрос к `search_coverage`, не цикл. Юниты не
перезапускались (нет указания владельца).
Доки: SQL › Functions › Utility Functions — query_table (снят с /health);
перепись — coverage_build.sql.
Числа: 3×vitrina 26,1 с; census 0,06 с; test_health_gap 23/23.

## 24.08: прогресс пересчёта считает движок, а не цикл psql [код] [замер]

[код] `embed_progress.sh:106-135` — находка снайпера коммита (п. 20 TARGET.md).
`count_parts` перебирал номера рабочих таблиц циклом `psql` (`seq 0..128`, один
`SELECT count(*)` на таблицу, суммирование в bash, конец набора угадывался по трём
промахам подряд). Теперь перечень таблиц спрашивается у движка — `duckdb_tables()`
с фильтром `current_database()`, — а сумма берётся ОДНИМ `query_table(<список>, true)`:
два вызова `psql` вместо N. Верхняя граница `EMBED_PART_WORKERS=128` как ограничитель
подсчёта снята: разрыв в нумерации больше не обрывает счёт.

[код] Неполнота перестала быть молчаливой (п. 13): в JSON появились
`parts_incomplete` и `parts_incomplete_reason` — каталог или сумма недоступны,
это видно в выводе, а не занижает числа «посчитано_всего»/«осталось».

[замер] Живая 26.07.3 (:7890): `estimated_size` у `duckdb_tables()` пустой — считать
по каталогу нельзя; `query_table()` принимает имена только литералом (подзапрос даёт
`Table function cannot contain subqueries`). Проба с пропущенным номером: две
part-таблицы `_part_0` и `_part_2` найдены обе, сумма 5, `parts_incomplete: false`.
Замок `test_embed_progress.py` — 13 проверок, итог OK.
Доки: sql/functions/duckdb_table_functions#duckdb_tables.

## 24.08: W-трек и rank-путь — три фикса окон + имя и ось лидера [код] [замер]

[код] W-трек (`serene_ask.py`, замер-драйв с боя okna 24.08, понедельник):
1. `period_readings` (~1284–1450): `drop_assumed` («без окна») входит в перечень
   прочтений ТОЛЬКО когда в разборе нет from/to; явный период лишнего класса не
   получает (стейджинг 23.08: clarify на «прошлом месяце» ушли).
2. `_is_seven_day_span`/`_assumed_sliding_week_not_calendar` (~1321–1348): assumed-окно
   ровно 7 дней, кончающееся сегодня/вчера и не совпадающее с текущей календарной
   неделей, → прочтения wtd (лидер) + full_week + исходное скользящее окно люком;
   разбор при этом подменяется на текущую пн–вс для раскрытия. Признак по границам
   и длине, без списков слов; prior и явные даты не трогаются.
3. Rank-путь: `sales_compare_intent` (~1555) больше не крадёт суперлатив
   («лучше всего») в compare; `rank_axis_resolve` (~3863) выбирает ось
   классификацией вопроса по именам осей источника (`rank_axis_pick` — модель
   видит только список имён осей, `kind_axis_hits`, `rank_axes_rerank` — штатный
   rerank по меткам; `product_axis_pref` со списками слов СНЯТ как хардкод);
   имя лидера — из результата GROUP BY (`rank_leader_atom` ~3934), текст кодом.
   Две+ правдоподобные оси → лидер + люк (§2 контракта), нет оси → честный
   clarify по осям, а не «нет данных в строках». `serene_axis.decide_grain`
   (~131): rank + 1 ось → group, много без hits → clarify=axis.

[замер] Оффлайн-замки: test_fork_window_readings **25→40**, test_rank_leader_path
(новый) **23→26**, test_rank_axis_anchor **66**, остальные наборы без изменений —
итого **486 + 26 + 66 зелёных**, 0 новых падений.

[замер] Живая цепочка на бое okna :8091 (24.08): «этот месяц» → B с MTD-лидером
(было FAIL в gold); «на этой неделе» → окно 2026-08-24..2026-08-24 = эталону;
«что лучше всего продавалось на этой неделе?» → kind=answer, op=rank,
«216981 Contraplaca bolt - cîrlig pentru ALU / 00000016397» = 2 — ровно эталон.
Финальная проба AB_PROBE: **8/8, 0 сбоев, средняя 34,27 с** (md5 `50b13be1`,
бэкапы `.bak-*` рядом). Доки: PLAN_ANSWER_CONTRACT §7bis (бизнес-справочники —
надстройка, решение владельца 24.08).
Числа: пробы 8/8; замки 486+26+66; gold «этот месяц» FAIL→OK; md5 50b13be1.

**[замер 24.08, кандидат перед коммитом]** После health-правки (запись выше
«/health не сканирует витрину») файл изменился: кандидат md5 **11d8f158** поднят
на стейджинге okna :8092 (копия в `/opt/1c-mcp-reports/serene_ask_probe.py`,
ASK_SQL_RRF=1 — та же конфигурация, что бой) и прогнан AB_PROBE с dev по туннелю:
**8/8, 0 сбоев, средняя 55,78 с**; name-эталон «216981 Contraplaca» = 2 — как в
бою. Отметка гейта `.probe-okna-last-run` обновлена прогоном (0err/8). 🔴
PROBE-строка scorera печатает md5 файла `PROBE_CODE_PATH` (умолчание
`/opt/1c-mcp-reports/serene_ask.py` на ДЕВЕ = 760c3923) — это НЕ md5 кандидата;
кандидат сверяется md5sum процесса на :8092.

## 23.08: лидер люка B/C — контракт 23.08 в коде [код] [замер]

[код] `fork_leader_class(picked[0], classes)` → `(leader, rest)` или `None`;
`fork_outcome_b`: text = одна пара лидера, options = N−1 не-лидерских классов,
atoms = [leader]+rest; `fork_outcome_c` (unsigned): число лидера +
`FORK_OTHER_READING`, без имён веток; `_journal_clarify_options` расширен на
B-люк (kind=figures); A без изменений.

[замер] Оффлайн-замки до/после: test_fork_outcomes **26→29**, test_leader_hatch
**13/0**, test_step4_guards **110/0**, test_fork_detector **43/0**,
test_b9_routing **9/0**, test_journal_fields **20→22/0**, test_gate **132/0**.

[замер] Живая приёмка на бою okna `:8091` (код выкачен, md5 **b31ef471**, бэкап
`.bak-08d2dc35`): AB_PROBE **8/8**, 0 сбоев, средняя **56,87 с**; `/health` **200**
за 71 с (холодный кэш gap — норма). Замороженные фразы `ab-gold-okna` (24 вопроса,
20:05 UTC): **12/24 — тот же счёт и тот же поимённый состав, что до выката**;
проверенные числа не сменились ни в одном вопросе (замок §10.14 выполнен).
Средняя выросла до **171 с**, два вопроса ушли в таймаут 600 с — GPU в тот момент
делили 27B и пересчёт klient-1; к коду отношения не имеет (часом раньше те же
пробы шли по 57 с).
Числа: пробы 8/8; gold 12/24=12/24 (состав совпал); health 200 за 71 с; md5 b31ef471.

## 23.08: Ф6.1 — красный бой объяснён, замер отложен до конца пересчёта [замер]

[замер] Разбор красного прогона 10:30 (5/24, 19 «сбоев»): **дефекта SQL-RRF там не
было**. Журнал okna: под флагом сервис ответил `200` на все дошедшие запросы, первые
5/5 верны. Провал устроила гонка — рестарт юнита 10:33:24 попал в середину замера:
вопрос 6 умер по сбросу соединения (20,3 с), вопросы 7–24 получили refused за 0,0 с,
пока новый процесс поднимался (~39 с). SereneDB в окне чист: ни ошибок, ни отказов прав.

[замер] Повтор без гонок (13:07–13:35, флаг в процессе, `/health` 200): **11/24,
средняя 68,24 с**, сбой 1 — против базлайна **12/24, 32,66 с** (10:15, флаг 0).
Сменился один вердикт: «сколько петель осталось на складе» `no_data`→`figures`.
Латентность ×2 — не от RRF: 27B делит GPU с пересчётом klient-1 (фаза corpus, util 100%).

[замер] Контрольный прогон флаг 0 в тех же условиях **невозможен**: на 49.13.97.101
слушает только `embed_server.py` :8002, **Qwen3.8-27B :8000 не запущен** (процесса vLLM
нет, порт закрыт) — ask отдаёт 503 `unavailable`, журнал: `ConnectionRefusedError` к
`10.3.1.12:8000`. Контроль дал 0/24 за 0,12 с — мерить было нечем.
Числа: 12/24@32,66s → 11/24@68,24s; вердикт сменился 1; LLM :8000 down; GPU 25,5/49 ГБ.

[решение] Ф6.1 на паузе до конца пересчёта klient-1; бой оставлен `ASK_SQL_RRF=0`,
юнит active. Приёмка Ф6.1 повторяется на прогретом сервисе при живом 27B и свободном
GPU: базлайн и замер подряд, без рестарта между ними.
...
12/24 (32,66 с) → **5/24 (7,46 с)**, DIFFS 24 — 19 ответов пустые (kind=—), fail-fast.
На стейджинге :8092 тот же флаг давал паритет 12/24=12/24 DIFFS 0 (0477aeb) →
дефект в окружении боя, не в SQL-RRF как таковом. /health 200 до и после, пробы 8/8.
Env боя откачен (ASK_SQL_RRF отсутствует), бэкап с флагом: env.bak-pre-f61-20260823-102849.
Обёртка сняла cursor-agent по сторожу тишины во время удалённого scorer (работа шла
на okna) — урок: длительные замеры гнать nohup на сервере + опрос.
Числа: 12/24→5/24; DIFFS 24; пустых 19; 32,66→7,46 с; /health 200; пробы 8/8.

## 23.08: root-пакет — dev на внешние IP GPU (vSwitch с dev недоступен, владелец) [решение]

[решение] `01-embed-env.sh`: хосты 10.3.1.11/10.3.1.12 → 178.63.211.188/49.13.97.101
(порты те же: 8000/8002/8001/8005); модель 4B и BATCH_CHARS=60000 без изменений.
README синхронизирован. Причина: с dev связи по внутренним адресам нет.
Числа: hosts 2; ports 8000/8002/8001/8005.
...
На новой okna (:2202) чистка следов миграции: удалён
`/var/lib/serenedb/engine_duckdb.bak-26073-20260822` (**~25,3 GiB**, апгрейд 26.08.1
подтверждён приёмкой); migrate-ключей `/root/.ssh/id_ed25519_migrate*` на момент
проверки уже не было; повторный `find` по `*migrate*`/`*bak-26073*` — пусто. Диск `/`:
20G used / 381G avail.
Числа: -25,3 GiB; disk 20G/381G.
...
`10.3.1.12`); пять эндпоинтов с коробки живы (embed×2 dim 1024, rerank, LLM
`Qwen3.8-27B`, whisper health 200). Пробы ask **4/4** по kind (`code_md5=d0945244`);
scorer-25 паритет старая=новая=**8/25** (регрессии переезда нет; провалы — дрейф дат
gold против снапшота 22.08 + известные просадки). Пересчёт 4B завершён: все не-service
строки — корпус **1 206 662**, резолвер **128 674**, карточки **254**; service-строки
**23 494+97** без вектора по политике. Временно остановленные 22.08 GPU-сервисы
(whisper/vLLM/diarize) возвращены владельцем. Канон эндпоинтов — `docs/NETWORK.md` §2.1.
Числа: в тексте.

## 22.08: Аудит: PostgreSQL как СУБД осталась только на dev [замер]

[замер] Проверены dev, старая okna (167.233.249.110), новая okna (10.10.10.12),
gpu-хост (178.63.211.188): пакеты postgresql-сервера, юниты, порт 5432, процессы,
каталоги — только на **dev** жив кластер postgresql-16 (127.0.0.1:5432, postmaster
active). На остальных — только клиентские пакеты (libpq5, postgresql-client) и psql
к SereneDB :7890. Юниты `@postgres` — имя инстанса SereneDB, не СУБД. Снос на
dev — в хвостах Ф3 (root, apt purge postgresql-16*).
Числа: в тексте.

## 22.08: GPU освобождён под пересчёт 4B — без ускорения [замер]

[замер] Временно остановлены (владелец разрешил; прод их не использует — проверено
env старой okna и живыми соединениями): whisper_server.py (gpu-1c pid 4346, :8006),
vLLM Qwen3.8-27B (gpu-qwen27b pid 162708, :8000), diarize_server.py (gpu-qwen27b
pid 48862, :8001). НЕ тронуты: rerank :8005 (прод RERANK_URL), embed :8000/:8002
(прод EMBED_HOSTS + пересчёт). До/после: суммарно ~320 req/min не изменилось;
VRAM gpu-qwen27b 48240→10350 MiB (−37,9 ГБ), gpu-1c 39800→29973 MiB. Вывод: упор
в compute embed-серверов, не в конкуренцию за VRAM. Рестарт после пересчёта —
cmdline в docs/RUNBOOK_MIGRATE_OKNA.md § «Временно остановленные сервисы GPU».
Числа: в тексте.
...
остатков снят; именованный товар без balance-источников → no_data до fork/модели; канон
продаж lock без picked; канон, пустой за период (вкл. assumed) → ответ 0.00 до выбора
сущности, а не clarify по мусорному источнику; ранг «лучше всего» — детерминированный
ответ с именем лидера. Карта search_balance_map в corpus_init.sql (GRANT) + бутстрап в
corpus_build.sql. Проба okna :8092: scorer 8/8, 0 ошибок (воскресенье-why=answer 0.00,
петли=no_data, неделя=answer с именем, прошлый месяц=2767450.98, прайс=2384); md5
serene_ask.py=d0945244. Замки: 27+74+63 зелёные.

## 22.08: env-флаг secondary_model не доезжал до kimi web — ловушка №5 [замер]

[замер] `/secondary-model` скрыта в сессии kimi web: флаг был только в `~/.profile`
(логин-шелл), а сервер запущен из интерактивного не-логин шелла — `/proc/<pid>/environ`
процесса kimi web флаг не содержит. Fix: экспорт продублирован в `~/.bashrc`;
рестарт сервера (не сессии) обязателен. Записано в ORCHESTRATION_CURSOR §7.2 и §9 №5.

## 22.08: Проба оркестрации — полный цикл Kimi → cursor-agent жив [замер]

[замер] Оркестратор породил `cursor-agent -p --force --model auto` (фон, 36 с, exit 0):
создан `work/orchestration-probe/hello.md` (3 строки), коммит `c70f75e` ровно из одного
файла, push в origin/main. Приёмка независимым замером оркестратора: md5 совпал, состав
коммита сверен, origin/main = c70f75e, дерево чистое.

[замер] Ловушка: промт со словами `git add`/`git commit` литералом в команде блокируется
снайпером (видит «коммит» при пустом индексе). Промт исполнителю — только из файла
(`.claude/state/prompt-<id>.md`). Записано в ORCHESTRATION_CURSOR §2.

[решение] HOW_NOT_TO большой и перед пачкой не читается: ловушки режима копятся
таблицей в самом регламенте — ORCHESTRATION_CURSOR §9, одна строка на поломку в том
же коммите, что и починка. Посеяно тремя замерами 22.08.

## 22.08: Ревизия AGENTS.md на мусор [замер]

[замер] Главная находка: **`read_section` у serenedb-docs починен на их стороне** —
раздел возвращается целиком (проверено живым вызовом на /cookbook/sql_features/merge);
примечание 02.08 про обход через WebFetch переписано с разбором, WebFetch — запасной.
Актуализированы протухшие числа/факты: проба 93 случая (было 89), граф ~700 КБ (было
~200), бинарь kimi — `~/.kimi-code/bin` (запасной синхронизирован), пометке про
`.mcp.json` дана дата замера (0.33). Проверено и оставлено: все ~30 ссылок на файлы
живы, «90 строк» шапки activeContext точно (MAX_LINES=90), прод — 26.07.3. Иной
структурной чистки файла правил не делалось — это ревизия правил, за владельцем.
...
за владельцем (§7 документа). Закрыто тем же днём: канон поставлен install-gates,
env-флаг в `~claudedev/.profile`, раздел «Режим оркестрации» вписан в AGENTS.md
(инод жёсткой ссылки сохранён, in-place правка).
...
**14/25, средняя 29,4 с, 0 сбоев** (было 9/25 на возврате-8, база 5/25).
Провалы сгруппированы: top-N/name по клиентам и товарам — 6 (много clarify
вместо ответа), сравнения периодов — 2, «не продаётся» — 1, «реально
покупают» (119 vs 141) — 1, «продали в этом месяце» — 1. Это классы 5–6
из `PLAN_ANSWER_CONTRACT` — следующий заход.

## 21.08: выкат возврата 12 на бой :8091 + ставка досчёта klient-1 [замер]

[замер] Выкат сделан оркестратором. Своя проба кандидата из HEAD на `:8092`:
**0err/8, 7/8 верных**. Единственный FAIL (`name`, «что лучше всего
продавалось») — не дефект: лидеры по Количеству за неделю делят 1668=1668
(`42084 Capac balama cercevea sus alb` из эталона и `42083 … toc sus alb`,
что выбрал кандидат) — оба ответа фактически верны, ничья ломается о
точное имя эталона. Целевые дефекты возврата 12 закрыты 4/4; прод `:8091`
после рестарта: июль → 2 767 450.98, «почему ноль» → answer с 0.00.

[замер] Боевой юнит на okna — `1c-serene-ask@postgres` (имя = dbname
движка), НЕ `@okna`: попытка restart `@okna` падает «Failed to load
environment files» (нет `/etc/1c-serene-ask-okna.env`), бой при этом не
страдает — живёт под `@postgres`.

[замер] klient-1 после фикса секретов: `ai_embed` 0,53 с/1024; ставка
досчёта по part-таблицам 11 791 → 13 547 за 416 с ≈ **4,2 строки/с**
(было ~0,3). Остаток без emb 12 963 млн (business-хвост ~7 млн) → ETA
при текущей ставке ~19–35 суток — вопрос параллелизма открыт.

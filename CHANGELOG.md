## 26.08: TARGET.md — п.14/16/17/20 по слову владельца [решение]

[решение] Контракт изменён владельцем 26.08 (коммиты 0b6dd3d, d4e6deb;
применено одноразовыми root-скриптами с проверкой якорей — файл root-only):
п.14 — инсталлятору вводится логин/пароль администратора базы 1С, читателя
в 1С и пользователя Windows создаёт сам инсталлятор; п.16 — наружу только
вызов DeepSeek для ответов OpenClaw (Alibaba убран); п.17 — свежесть «как
можно быстрее», фиксированного срока нет (на огромной базе эмбеддинг не
успевает в заданное окно); п.20 — эмбеддинг штатным ai_embed движка.
Таблица проверок 14/17 синхронно. Синхронизированы TARGET_STATUS (п.14–17),
PLAN_TO_TARGET (И0 без порога 20 мин). Остаток рассинхрона (доки с egress
Alibaba) зарегистрирован в activeContext.

## 26.08: замер — ai_embed жив в SereneDB 26.08.1; версии движков [замер]

[замер] По слову владельца (правки TARGET.md п. 14/16/17 + «проверить вызов»
по п. 20): `ai_embed(text, model, secret_name)` существует в живой сборке —
вызов на контуре 17890 дошёл до проверки секрета (`secret 'local_ai' not
found`), т.е. функция нативно зарегистрирована. Доки: «AI Functions —
Sql › Functions › AI Functions». Движок на 17890 — **SereneDB 26.08.1**
(в доках проекта стояла 26.07.3 — поправить при ближайшей правке
techContext/SERENEDB). Dev `:7890` подтверждённо мёртв: timeout даже на
connect (pg_isready при этом отвечает «accepting» — ESTAB 337 висят).

Доки: AI Functions — Sql › Functions › AI Functions (docs.serenedb.com/sql/functions/ai).

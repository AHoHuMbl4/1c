...
- **Порядок держится на приборах, а не на важности** `[решение]`: сперва стенд (пока
  ...
  (`HOW_NOT_TO §1.27`).
- **Все 47 вопросов «(в)» решены разработкой** `[решение]`: под каждым в
...
- **Владельцу оставлено семь пунктов** `[решение]` — только то, что меняет контракт или
...
- **Перечитано свежим взглядом против входных файлов** `[код]`: перечит поймал четыре

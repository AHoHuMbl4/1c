...
- **Маршрутизация по CN клиентского серта (08.08):** `CN=medteh-ut` →
  `packet_unit_medteh_ut` (юнит `10.1.1.5:6090`, приватная сеть); остальные CN —
  дефолтный backend (туннель на дев). `[замер 08.08]`: с сертом `medteh-ut`
  `GET /v1/agent/config?base_id=medteh-ut` вернул конфиг юнита (дев ответил бы
  401 — маршрут доказан), с сертом `ut` — по-прежнему дев (`packet-server-ok`).

**FreeBSD `201.34.130.46` — ⛔ ВЫВЕДЕНА из контура 08.08** (DNS ушёл на
Ubuntu-релей, туннельный юнит `1c-gate-tunnel.service` остановлен и выключен
из автозапуска, VM удалена владельцем 08.08). Исторически (06.08–08.08) держала:

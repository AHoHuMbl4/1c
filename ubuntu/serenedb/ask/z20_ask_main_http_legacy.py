"""БЫВШИЙ главный обработчик ask (LEGACY; решение владельца 12.09, вариант B).

Файл переименован на время переписывания тракта «один путь»:
вопрос → интерпретация из вики → запрос в базу → меню при >1 прочтении →
выбор человека → ответ. Новый тракт пишется в z20_ask_main_http.py; этот
файл — источник переноса (HTTP-оболочка, билеты, дедлайн, compose-хвост)
и эталон регрессии до flip. После приёмки нового тракта файл удаляется.
Карта остатков: docs/audit/onepath/O1-map.md; проект: O2-project.md;
дыры к закрытию: O3-red.md №8-14; судьба замков: O5-locks.md."""

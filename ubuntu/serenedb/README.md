# SereneDB — установка (self-host, LXC .42)

SereneDB — search+analytics БД (полнотекст + вектор + колоночный OLAP) с **Postgres-протоколом**,
Apache-2.0 (single-node). У нас — движок витрины/аналитики/семантического резолвера над данными 1С.
Подробнее и план — в `docs/SERENEDB.md`.
...
- **Бэкап/restore** штатно в доке не описан — открытый вопрос (тест + вопрос фаундерам). Смягчается
  тем, что витрина — **производная от 1С**, пересобирается из источника.
- Загрузка данных: обычный SQL + `CREATE TABLE AS SELECT read_csv/read_parquet(...)` (вывод ETL).

#!/usr/bin/env python3
"""Оффлайн: сторож corpus_merge — смена формы ключа vs потеря (п.13).

100 % старых ключей сущности + непустая новая сборка не меньше → пропуск с записью;
частичный уход или новая меньше старой → STOP.
...
def classify_entity(было, уйдёт, стало):
    """Логика tmp3_merge_ent_guard + tmp3_merge_key_form (без анти-соединений)."""
...
def guard_count(unmatched_by_ent, key_form_ents):
    total = sum(unmatched_by_ent.values())
    exempt = sum(n for ent, n in unmatched_by_ent.items() if ent in key_form_ents)
    return total - exempt
...
t("partial: 50 из 100", classify_entity(100, 50, 120) == "partial_stop")
...
t("guard: без key_form весь объём",

...
MEANING_TOP = int(os.environ.get('ASK_MEANING_TOP', '8'))
...
def aggregate(src_table, match, preds, measure=None):
    ...
    r = psql("SELECT count(*) FILTER (%(nf)s), coalesce(sum(%(m)s) FILTER (%(nf)s),0), "
             "       coalesce(min(%(m)s) FILTER (%(nf)s),0), "
             "       coalesce(max(%(m)s) FILTER (%(nf)s),0), "
             "       coalesce(avg(%(m)s) FILTER (%(nf)s),0), "
             "       coalesce(min(doc_date)::date::text,''), "
             "       coalesce(max(doc_date)::date::text,''), "
             "       count(%(m)s) FILTER (%(nf)s), "
             "       count(*) FILTER (NOT (%(nf)s)) "
             "FROM %(src)s WHERE %(w)s"
             % {"m": m, "src": src, "w": " AND ".join(where), "nf": folder_pred})
...
def answer(question, focus=None, measure_pick=None, context="", no_arbiter=False):
    ...
    extra = []
    for src_from in (alias_hits(exprs, MEANING_TOP),
                     near_tables(intent.get("kind") or question, MEANING_TOP)):
        for t in src_from:
            if t not in by and t not in extra:
                extra.append(t)
    if extra:
    ...
    def _alias_verdict(cand):
        ...
        if ALIAS_VETO and top:
            return _family(cand) == _family(top[0][0]), top
        if hit:
            return True, []
        return False, top
    ...
    if plan.get("quantity") and plan["quantity"] in measures_of(src):
        ...
        # 🔴 ГЕЙТ ВЕЛИЧИНЫ (задача 15 реестра «право на ответ (б)»). Имя, названное моделью,

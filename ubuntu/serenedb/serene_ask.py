...
def answer(question, focus=None, measure_pick=None, context="", no_arbiter=False):
    ...
    # Кандидаты, у которых спрошенное поле есть в данных. Используется ТОЛЬКО как источник
    # соперника для арбитра — ни порядок отбора, ни состав кандидатов не меняются.
    # [замер 30.07] перестановка вредна тоже: подняв «есть поле» в начало круга, я сломал
    # «сколько партнёров», который до этого отвечал верно. Единственная безопасная форма
    # признака — добавить соперника, не сдвинув никого (`HOW_NOT_TO §3.30`).
    field_ok = set()
    if FIELD_MUST_EXIST and intent.get("kind") and len(cands) > 1:
        try:
            ...
            # Годится и то, и другое: поле `Имя:` в тексте ЛИБО название сущности.
            ...
            # 🔴 НАЗВАНИЕ СРАВНИВАЕТСЯ ПО ОСНОВАМ СЛОВ, ШТАТНЫМ СЛОВАРЁМ ДВИЖКА, А НЕ
            # ПОДСТРОКОЙ. Указание владельца 30.07: «разве это не должно было решиться через
            # средства serene? там ведь инструмент, чтобы „спб" был Санкт-Петербург».
            # Инструмент есть и был у нас описан (`SERENE_CAPS_3 §2.4`), а я написал
            # `contains()` — и он ломался на падежах:
            # [замер 30.07] `contains('склады','складов')` ЛОЖЬ в обе стороны, поэтому
            # `catalog_склады` выпадал, а уцелевали константы `ИспользоватьНесколькоСкладов`,
            # где словоформа лежит целиком и случайно. Со словарём: обе стороны -> `{склад}`.
            # Тот же приём снимает и монетку `закупки`/`закупка` -> `{закупк}`.
            # Словарь наследует локаль базы (`copy_from`), своего значения я не задаю —
            # список окончаний или слов был бы привязкой к языку (девиз, п. 12).
            try:
                has_field |= {r[0] for r in psql(
                    "SELECT src_table FROM %s WHERE src_table IN (%s) "
                    "  AND list_has_any(ts_lexize(%s, label), ts_lexize(%s, %s))"
                    % (TABLES, ", ".join(lit(c) for c in cands),
                       lit(STEM_DICT), lit(STEM_DICT), lit(intent["kind"]))) if r and r[0]}
            except RuntimeError:
                pass
            ...
            if has_field:
                ...
                field_ok = has_field       # порядок отбора НЕ ТРОГАЕМ
        except RuntimeError:
            pass

        ...
        try:
            ...
            if r is None:
                ...
                if r["rows"] == 0:
                    ...
                    ok += 1
                    # 🔴 ПОЛНАЯ ЗАГРУЗКА — ЭТО ТОЖЕ ИЗМЕНЕНИЕ ВИТРИНЫ. Она безусловно
                    # перезаписывает таблицу, и знать, совпало ли содержимое, мы дешёвым
                    # способом не можем. Прежде счётчик рос только у дельты, а дельта
                    # неприменима к 80% источников (нет пары `Ref_Key`+`DataVersion`):
                    # [замер 30.07] 1 202 из 1 502 на второй базе, 106 из 231 на первой.
                    # Изменения регистров и табличных частей не ставили отметку, сборка их
                    # пропускала — та же потеря свежести (п. 17), что чинилась днём, но
                    # через вторую дверь и для большей части базы.
                    touched += r["rows"]

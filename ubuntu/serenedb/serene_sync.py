...
def _service_skip(ents):
    ...
    признаков четыре, и они разной природы:

      1. `only_binary` — объективный, из `$metadata`, ничьего суждения не требует;
      2. тень `_RecordType`/`_RowType` — суффикс платформы OData (corpus_build.sql),
         не список имён; данные теней в ответы не входят;
      3. разметка `search_entity_class = 'service'` — смысловая, её кладёт модель один раз
         на сущность и хранит в базе; та же, по которой владелец 29.07 отменил векторы
         служебным;
      4. `search_entity_force` — слово владельца, перебивает остальные: строка `skip`
         убирает источник, строка `load` возвращает его, что бы ни решили первые.
    ...
    for es in ents:
        ...
        elif es.endswith("_RecordType") or es.endswith("_RowType"):
            # Суффикс платформы OData 1С (corpus_build.sql:192-197), не список имён.
            # Строки теней в корпус/ответы не входят; имена — из снимка $metadata
            # (balance_registers), без строк витрины.
            skip[es] = "тень регистра OData (_RecordType/_RowType, $metadata)"
...
def main():
    ...
    # 🔴 СПИСОК ПОМЕЧАЕТСЯ НЕДОСТОВЕРНЫМ ДО РАБОТЫ, А НЕ ПОСЛЕ. Если синк оборвётся посреди
    # цикла — упадёт процесс, кончится память, погасят юнит, — прежний список останется в
    # базе, и без этой отметки сборка сочла бы его свежим и пересобрала только «изменённое»
    # по вчерашним данным. Признак снимается последней командой синка: он же и доказывает,
    # что синк дошёл до конца.
    try:
        subprocess.run(["psql", dsn, "-q", "-v", "ON_ERROR_STOP=1", "-f", "-"],
                       input="DELETE FROM search_quality WHERE k = 'changed_sources_ok';\n"
                             "INSERT INTO search_quality VALUES ('changed_sources_ok', 0, "
                             "'синк идёт — список изменившихся таблиц ещё не готов');",
                       capture_output=True, text=True, check=True)
    except Exception as e:                      # noqa: BLE001
        print(f"⚠ не удалось пометить список изменившихся недостоверным: {str(e)[:120]}")
    ...
    for es in ents:
        try:
            ...
            err += 1
            # 🔴 УПАВШАЯ СУЩНОСТЬ СЧИТАЕТСЯ ИЗМЕНИВШЕЙСЯ, А НЕ ПОРТИТ ВЕСЬ СПИСОК. Дельта
            # могла упасть уже ПОСЛЕ того, как тронула таблицу, — значит про эту таблицу мы
            # не знаем ничего и обязаны дать сборке перечитать её. Но только её.
            #
            # Прежде здесь стояло «была ошибка — весь список недостоверен», и это
            # оказалось слишком грубо: [замер 06.08, первая база] один источник закрыт
            # правами (`HTTP 401`), и такая ошибка не разовая, а постоянная — экономия
            # отключалась бы на этой базе НАВСЕГДА из-за одной недоступной сущности.
            changed_tables.add(L.safe_col(es).lower())
            try:
                L._upsert_changed_rows(L.safe_col(es).lower(), ["*"], "full")
            except Exception as mark_err:  # noqa: BLE001
                print("  %s: маркер full не записан: %s" % (es, mark_err))
            print(f"  {es}: ОШИБКА {e}")

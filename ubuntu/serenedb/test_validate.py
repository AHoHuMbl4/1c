...
def _pick_table():
    ...
    for t in tabs:
        ...
        if n.isdigit() and int(n) > 0:
            return t
    # Запасного ИМЕНИ здесь быть не может: любое имя — из нашей базы, и на чужой
    # конфигурации тест молча проверял бы несуществующую таблицу, показывая зелёный.
    # Нет витрины — нет и предмета проверки, это отказ, а не подстановка.
    if not tabs:
        raise SystemExit("нет ни одной таблицы витрины — проверять нечего")
    return tabs[0]

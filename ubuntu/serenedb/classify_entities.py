...
🔴 ЭТО ПРИОРИТЕТ, А НЕ ИСКЛЮЧЕНИЕ. Ни одна строка не выбрасывается:
  ...
  * векторы считаются сначала бизнес-сущностям, потом служебным;
  * при обрыве такта без вектора остаётся служебное, а не то, о чём спрашивают.
...
def main():
    ...
    # Остаток безопасно достаётся следующими тактами: неразмеченное трактуется как
    # `business`, то есть до разметки сущность попадает в ПЕРВЫЙ проход векторизации, а не
    # выпадает из неё. Ошибка возможна только в сторону «посчитали раньше времени».

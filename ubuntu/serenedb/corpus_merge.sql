...
-- 🔴 ОБРЫВ = витрина НЕ ПОМЕЩАЕТСЯ в сборку (ключей витрины БОЛЬШЕ строк
-- сборки); сборка больше витрины — «#N»-дубли, не потеря (живой стоп 22:00:
-- установкацен 323570 строк против 322071 ключей при 1499 дублях — было
-- 646862, усохший источник с дублями; «<>» ронял ложно). Усохший источник
-- подтверждён: витрина целиком представлена в сборке.
SELECT 'SELECT CASE WHEN (SELECT count(DISTINCT (' || kexpr || ')) FROM query_table('
       || quote_literal(src_table) || ')) > ' || стало
       || ' THEN error(''corpus_merge: новая сборка меньше старой И меньше витрины: '''
       || ' || ' || quote_literal(src_table)

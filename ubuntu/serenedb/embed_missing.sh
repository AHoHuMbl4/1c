...
# 🔴 РАЗМЕР ПАЧКИ — РУЧКА, А НЕ КОНСТАНТА. Десятка была пределом прежнего облачного
# провайдера («batch size is invalid» на большем), у своего сервиса такого предела нет.
# [замер 02.08] на настоящих строках корпуса (в среднем 379 символов, один поток):
# пачка 10 — 7,0 строк/с, 16 — 9,5, 32 — 8,4, 64 — 6,2. То есть и слишком мелкая, и
# слишком крупная одинаково плохи: первая тратится на накладные расходы запроса, вторая
# упирается в токены на один проход. Значение подбирается замером на СВОИХ данных.

...
# 🔴 Но раздельные таблицы В ОДНОЙ БАЗЕ конфликт НЕ снимают, как утверждала прежняя
# версия: писатель у базы один, и пишущие операторы всё равно идут по очереди — вместе с
# запертым внутри них сетевым вызовом. [замер 20.08] 18 воркеров в 18 разных таблиц одной
# базы дали 15,2 строк/с при ОДНОМ активном запросе. Настоящая одновременность требует
# отдельной присоединённой базы на поток (`ATTACH '...' AS wN (ROW_GROUP_SIZE ...)`):
# с ней те же 18 потоков дали 240-354 строк/с. Здесь этого нет намеренно — такт считает
# дельту, ему хватает; так делается только разовая сборка (`docs/EMBED_BULK_HOWTO.md`).
#
...
psql_q -c "CREATE OR REPLACE TABLE ${TAG}_todo AS
  ...
  SELECT * EXCLUDE (rn, cum),
         ...
         -- БОЛЬШЕ положенного строк. [замер 29.07] провайдер отвергал такие пачки целиком:
...
# 🔴 ПОТОКИ ДВИЖКА — ЭТО И ЕСТЬ ПОТОЛОК ЧИСЛА ОДНОВРЕМЕННЫХ ВЫЗОВОВ МОДЕЛИ.
# `ai_embed` держит поток исполнителя всё время сетевого запроса, поэтому наружу уходит
# ровно `threads` запросов — сколько бы соединений psql мы ни открыли. Доки говорят это
# прямо и советуют для многих мелких сетевых запросов ставить потоков в 2-5 раз больше
# ядер: https://docs.serenedb.com/cookbook/performance/how_to_tune_workloads#querying-remote-files
# [замер 19.08, klient-1] при `threads=4` и ДВЕНАДЦАТИ воркерах в полёте держалось 1-4
# запроса, а резолвер шёл 75,8 строк/с: восемь воркеров просто стояли в очереди.
#
# 🔴 ПОДНИМАЕМ ТОЛЬКО НА ВРЕМЯ ДОСЧЁТА И ВОЗВРАЩАЕМ ОБРАТНО. `threads` глобален и действует
# на сборку корпуса тоже, а там поток стоит памяти — на клиентском юните это уже роняло
# движок в OOM. Список работы (окно по всей таблице) строится ВЫШЕ, на прежнем значении;
# здесь остаётся только сеть и мелкая вставка. Без `EMBED_THREADS` движок не трогается
# вовсе — поведение прежнее.
THR_OLD=""
restore_threads() {
...
if [ -n "${EMBED_THREADS:-}" ]; then
  ...
  if [ -n "$_thr_now" ] && [ "$_thr_now" -lt "$EMBED_THREADS" ]; then
    if psql "$DSN" -q -c "SET threads = $EMBED_THREADS" >/dev/null 2>&1; then
      THR_OLD="$_thr_now"

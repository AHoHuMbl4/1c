...
-- 🔴 02.08: исключения «длиннее предела эмбеддера» здесь БОЛЬШЕ НЕТ, и это не послабление
-- счёта, а следствие правки в `build.sh`: вход эмбеддера теперь обрезается
-- (`substr(doc,1,EMBED_MAXLEN)`), поэтому вектор возможен КАЖДОЙ строке, и «не убудет
-- никогда» перестало быть правдой. Пока условие стояло, длинные строки не попадали ни в
-- работу, ни в счёт — то есть их отсутствие в поиске по смыслу было невидимо здесь.
...
SELECT CASE WHEN ts_lexize(:'solr_syn_dict', 'x') IS NULL
       THEN error('словарь ' || :'solr_syn_dict' || ' не отвечает ts_lexize — '
                  || 'Ф6.3 query-side synonyms сломан') END;

  ...
  while [ "$rounds" -lt "${WIKI_ALIAS_COLLISION_ROUNDS:-40}" ]; do
    ...
    psql "$DSN" -tA -c "
      WITH al AS (SELECT src_table, trim(lower(x.a)) AS alias
           ...
           -- 🔴 Но можно назвать слово прямо (`WIKI_ALIAS_WORD`) — это быстрый путь, когда

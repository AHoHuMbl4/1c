...
JSON only, no prose, no code fences. Below are record types of one database, shown together because they are CLOSE IN MEANING — that is what makes them easy to confuse. Answer language for every string = the language of that record's title (the English example below is STRUCTURE ONLY — never copy its language when the title is in another language). TASK: for EACH record produce ONLY the aliases field — everyday words a person actually puts in a question when they mean THIS kind of record (their spoken asking-words). RULES: (1) Include ROLE words for every flow listed in the input for this record: the same catalog is named differently by role depending on the flow (structure example: a counterparty catalog → buyers in sales flows, suppliers in purchase flows); take roles ONLY from the listed flows — do not invent flows. (2) WHEN the input for this record lists flows OR movement / money-total / event totals, not a bare headcount/Count alone: at least ONE alias MUST be a spoken action/event form people use when asking about such events (same language as the title, 1-2 words). Keep that event alias even if a sibling could use a similar word — the sibling distinction belongs to the notEnoughFor field (a separate call), not to dropping the event alias. If you must drop something to fit the limit, drop a redundant noun synonym, never the event alias. For records without flows/quantities, event aliases are optional — include only when natural. (3) Limits: 3 to 10 aliases; each alias is 1 to 3 words; no sentences; no question words alone (how/how many/what/who as standalone tokens). (4) Do NOT add the title (or a morphological variant of the title) as an alias — the title is already known from input. Do not put quantity names or field names into aliases (quantities are filled by a separate call). Do not copy raw dimension labels that are opaque metadata; use spoken asking-words for those axes. (5) BANS (skip the token if unsure): platform meta-labels and their equivalents in the title language (list, catalog, directory, types, kinds, register, journal, document, classifier, form, report as a meta-word); case/number/inflection variants of the SAME stem (pick one citation form); Latin-script words when the title is not Latin; NOUN words that equally fit a sibling in THIS batch (leave those out — the distinction goes to notEnoughFor), EXCEPT spoken action/event forms (rule 2) and AXIS subject nouns under rule 7; jargon opaque to a non-developer. (6) "If unsure — omit" applies to meta-labels and jargon; for action/event forms on records meeting the event trigger (rule 2): prefer one everyday event form over a near-duplicate noun synonym. (7) AXIS NOUNS FROM axes (event records only): WHEN this record's Input lists a non-empty axes field AND meets rule 2's event trigger: include 1-3 everyday spoken nouns (1-3 words each, citation form, title language) paraphrasing those axes — people put these nouns in ranking / breakdown questions that THIS record answers. Paraphrase axis labels into the words a person actually says; never paste an Input axes string verbatim. Take axes ONLY from the listed axes — do not invent axes. Records without the event trigger (catalogs and other master lists) get NO forced axis nouns even when their axes field is non-empty — their axes describe their own attributes, not ranked events. If axes is empty or absent on an event record: do not add axis nouns from imagination; optionally apply the V1 sibling-title cue ONLY when rule 2's event trigger also holds. Keep axis nouns even if a sibling catalog equally fits them — contrast goes to notEnoughFor. Drop order under the 10-cap: redundant title synonyms first, then lower-priority axis nouns, never the mandatory event alias. (8) SAME-TITLE SIBLINGS: when two or more Input items share the same or near-identical titles, EACH of them MUST also carry at least one DISTINCTIVE everyday alias naming its own nature (structure example: one concrete paper per row vs accumulated period totals vs line-item detail), so a person can name the right one with a whole asking-word — this difference must live in aliases, not only in notEnoughFor. Structure example (English skeleton only): {"items":[{"entity":"doc_sales","aliases":["sales","shipments","goods","customers"]}]} Never copy the English example strings into the output when the title language is different. Every Input entity appears once; entity values copy Input exactly. Schema: {"items":[{"entity":"<exact copy of Input entity string>","aliases":["..."]}]}. Input:
EOF_WA_PROMPT
)
_WA_INIT_B=$(cat <<'EOF_WA_PROMPT'
JSON only, no prose, no code fences. Below are record types of one database, shown together because they are CLOSE IN MEANING. Answer language for every string = the language of that record's title (the English example below is STRUCTURE ONLY). TASK: for EACH record produce ONLY the bestUsedFor field — 2 to 4 short question TEMPLATES this record truly answers (the shape of what a person asks, not full sentences). RULES: (1) No foreign topics (do not advertise price-list / stock-balance / customer-count questions for a record that does not answer them). (2) No office jargon gerunds. Prefer spoken wording over metadata wording. (3) Hard format rule for EVERY string: no PARENTHESES WITH A COMMA INSIDE the string (parentheses without a comma, and commas outside parentheses, are fine). Rewrite enumerations with a dash or a word instead. (4) RANK TEMPLATES (event records only): WHEN this record has flows OR movement / money-total / event totals, not a bare headcount/Count alone: at least ONE template MUST be ranking / top-N / best / most in the title language, using a spoken axis noun paraphrased from axes when axes is non-empty, otherwise one generic ranking-by-totals template. Templates must reflect THIS record's role among Input siblings sharing the same axes (accumulated totals for a period / one concrete document / line-item detail / a specialized flow) — never copy one identical ranking template across siblings; a bare axis noun is an addition, never this record's only marker. Prefer covering different axes across the 2-4 slots when several axes exist — still max 4 strings total. Keep at least one non-rank total/period template when natural. Records without the event trigger: do not force rank templates. Structure example (English skeleton only): {"items":[{"entity":"doc_sales","bestUsedFor":["sales total for a period","top goods by amount","who bought the most"]}]} Never copy the English example strings when the title language is different. Every Input entity appears once; entity values copy Input exactly. Schema: {"items":[{"entity":"<exact copy of Input entity string>","bestUsedFor":["..."]}]}. Input:
EOF_WA_PROMPT
)
_WA_INIT_C=$(cat <<'EOF_WA_PROMPT'
JSON only, no prose, no code fences. Below are record types of one database, shown together because they are CLOSE IN MEANING — that is what makes them easy to confuse. Answer language for every string = the language of that record's title (the English example below is STRUCTURE ONLY). TASK: for EACH record produce ONLY the notEnoughFor field — short strings telling what this record does NOT answer. KINDS, required when applicable: (a) THEMATIC "not me" — topic labels a person might confuse with this record but that this record does NOT answer (stock balances, price list, customer list/count, cash, payroll, … — only topics that are plausible confusions for THIS title); (b) SIBLING redirect — for each easy-to-confuse sibling from THIS input list, one short string: sibling title then what THAT sibling answers instead. When a sibling shares this record's action/event asking-words, name that shared topic here (what THAT sibling's events are). (c) AXIS vs EVENT using axes: when THIS record meets the event trigger (flows OR movement / money-total / event totals, not a bare headcount/Count alone) and a sibling's title matches one of THIS record's axes, say THIS is not the axis master list — see sibling title; when THIS record is a catalog whose title matches an axis of an event-triggered sibling, say THIS is not ranked event totals on that axis — see sibling title. If no event-triggered sibling shares the axis: omit kind (c). FORMAT RULES: no commas and no parentheses inside any string (downstream stores arrays as comma-CSV). Use a dash or the word "not"/"see" instead. Prefer 3-8 strings total. If unsure whether a topic is a real confusion — omit it. Do not invent sibling names that are not in the input. Structure example (English skeleton only): {"items":[{"entity":"catalog_goods","notEnoughFor":["not stock balances","not ranked sales totals - see Sales"]},{"entity":"doc_sales","notEnoughFor":["not the goods catalog itself - see Goods"]}]} Never copy the English example strings when the title language is different. Every Input entity appears once; entity values copy Input exactly. Schema: {"items":[{"entity":"<exact copy of Input entity string>","notEnoughFor":["..."]}]}. Input:
EOF_WA_PROMPT
)
_WA_COLL_A=$(cat <<'EOF_WA_PROMPT'
You are a database disambiguation engine. JSON only, no prose, no code fences. The record types in Input are ALL CALLED BY THE SAME WORD in this database. That shared word is "<SHARED_WORD>": a person who types only that word could mean any type below. Answer language = the language of each title. TASK: for EACH type produce ONLY the aliases field. RULES: (1) Keep "<SHARED_WORD>" in the list — people use it. Also keep the title if it is already an everyday asking-word; each alias is a distinct word — no grammatical variants of the same stem. (2) ADD 1-2 DISTINCTIVE everyday words or short phrases (1-3 words each) that a person would use when they mean THIS type and not the siblings. WHEN the input for this type lists flows OR movement / money-total / event totals, not a bare headcount/Count alone: at least ONE of the added words MUST be a spoken action/event form people use when asking about such events (same language as the title). Keep that event word even if a sibling could use a similar one — the sibling distinction belongs to the notEnoughFor field (a separate call), not to dropping the event word. If only 1 confident — add just 1; quality over quantity. (3) Ground aliases only in this type title, its listed flows, quantities, and axes if Input lists them (and in its current aliases if Input lists them); take roles/event cues from listed flows and quantities only — do not invent flows or quantities. (4) Distinctive phrases may stay unique to this type, or overlap a sibling only when both truly share that role; empty contrast (only the shared word and bare title for every sibling) leaves siblings indistinguishable — give contrast with whole asking-words, not truncated title fragments. (5) Aliases are only concrete everyday words a person would type for THIS type. Match the title language exactly. When unsure a word is everyday for THIS type — omit it (this "unsure — omit" applies to noun words; for action/event forms on types meeting the event trigger (rule 2): prefer one everyday event form over a near-duplicate noun synonym). (6) When "<SHARED_WORD>" is an AXIS subject noun and THIS type has flows OR movement/money/event quantities (not bare Count): keep "<SHARED_WORD>" AND the event form; do not replace the shared axis noun with only a verb. When THIS type is the axis catalog, keep "<SHARED_WORD>" as catalog asking-word; distinctive adds describe the master-list sense not the ranking-totals sense. Ground axis cues in title, flows, quantities, current aliases, and axes if Input lists them — do not invent. (7) PRESERVE AXIS/EVENT TOKENS: when producing this type's aliases, keep the axis-subject nouns and spoken event forms already present in this type's Input aliases — they were added deliberately; drop plain duplicates and redundant title synonyms first, never the event form or the axis nouns. Few-shot (English scaffold only; YOUR output language = language of each title). Shared word "party": {"items":[{"entity":"ent_partners","aliases":["party","partners","buyers"]},{"entity":"ent_counterparties","aliases":["party","counterparties","legal entities"]}]}. Schema: {"items":[{"entity":"<exact copy of Input entity string>","aliases":["..."]}]}. Every Input entity appears once; entity values copy Input exactly. Input:
EOF_WA_PROMPT
)
_WA_COLL_B=$(cat <<'EOF_WA_PROMPT'
You are a database disambiguation engine. JSON only, no prose, no code fences. The record types in Input are ALL CALLED BY THE SAME WORD in this database ("<SHARED_WORD>"). Answer language = the language of each title. TASK: for EACH type produce ONLY the bestUsedFor field — 2 to 4 short topic templates this type alone answers among the siblings (short phrases, not full sentences; topics belong to this type, not to a sibling). RULES: (1) No foreign topics (do not advertise price-list / stock-balance / customer-count questions for a type that does not answer them). (2) No office jargon gerunds. Prefer spoken wording over metadata wording. (3) FORMAT RULE: no PARENTHESES WITH A COMMA INSIDE any string (parentheses without a comma, and commas outside parentheses, are fine). Rewrite enumerations with a dash or a word instead. (4) When THIS type has flows OR movement / money-total / event totals, not a bare headcount/Count alone: at least one bestUsedFor template MUST be ranking / top-N / best / most that THIS type alone answers among the siblings (title language), reflecting THIS type's role — never copy one identical ranking template across siblings. The axis-catalog sibling must NOT advertise those ranked event totals. Few-shot (English scaffold only): {"items":[{"entity":"ent_partners","bestUsedFor":["who we sell to","partner headcount"]},{"entity":"ent_counterparties","bestUsedFor":["taxpayer id","contract party"]}]}. Schema: {"items":[{"entity":"<exact copy of Input entity string>","bestUsedFor":["..."]}]}. Every Input entity appears once; entity values copy Input exactly. Input:
EOF_WA_PROMPT
)
_WA_COLL_C=$(cat <<'EOF_WA_PROMPT'
You are a database disambiguation engine. JSON only, no prose, no code fences. The record types in Input are ALL CALLED BY THE SAME WORD in this database. That shared word is "<SHARED_WORD>": a person who types only that word could mean any type below. Answer language = the language of each title. TASK: for EACH type produce ONLY the notEnoughFor field — when a person says "<SHARED_WORD>" but means a COMPETITOR TOPIC that another type in this list answers better, name that TOPIC (what they are looking for), then optionally the sibling title from Input. RULES: (1) Prefer themes over bare sibling names. Example shape: "stock on hand / warehouse balances — see <sibling title>"; "current price list — see <sibling title>". (2) Siblings come only from Input. (3) Each type includes the shared word "<SHARED_WORD>" in at least one notEnoughFor entry so later passes see the collision was addressed. (4) When a sibling shares this type's action/event asking-words, name that shared topic here (what THAT sibling's events are). (4b) When the shared word is an axis subject: the catalog type's notEnoughFor names ranked event totals / top-N — see the event sibling title; the event type's notEnoughFor names the axis master list — see the catalog sibling title. Still include "<SHARED_WORD>" in at least one string per type (rule 3). Write rank phrases with dashes exactly like the examples - never comma lists (a string like 'top-N, ranked' is a hard format failure). No commas or parentheses. (5) FORMAT RULE: no commas and no parentheses inside any string (downstream stores arrays as comma-CSV). Use a dash or the word "not"/"see" instead. (6) When unsure — omit that line. Few-shot (English scaffold only; YOUR output language = language of each title). Shared word "party": {"items":[{"entity":"ent_partners","notEnoughFor":["party as legal taxpayer / contract party — see Legal counterparties"]},{"entity":"ent_counterparties","notEnoughFor":["party as sales partner list — see Business partners"]}]}. Schema: {"items":[{"entity":"<exact copy of Input entity string>","notEnoughFor":["..."]}]}. Every Input entity appears once; entity values copy Input exactly. Input:
EOF_WA_PROMPT
)

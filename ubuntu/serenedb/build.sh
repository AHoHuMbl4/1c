...
# кэшем в базе. Нужна ДО векторизации. 🔴 FAIL-CLOSED (возврат 2 / 21.08): если модель
# недоступна (401/404/timeout на весь прогон) — такт СТОП. Иначе при пустом class
# условие «НЕ service» истинно для всех строк и GPU считает служебное (дырка klient-1:
# 5,5 млн ГАР/ФИАС). Потолок CLASSIFY_MAX_PER_RUN — не ошибка: classify выходит 0,
# остаток доразметится следующими тактами.
...
# Служебным метка-вектор тоже не нужна: отбор кандидатов их и так отсекает по class.
...
# Service не резолвим и не векторизуем (DATA_SCOPE §9.4 / возврат 2).
ROWS_WHERE="NOT EXISTS (SELECT 1 FROM search_entity_class e

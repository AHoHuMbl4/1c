...
echo "== 3. векторы новым строкам корпуса — сначала бизнес-сущности"
ROWS_WHERE="src_table IN (SELECT src_table FROM search_entity_class WHERE cls = 'business')
            OR NOT EXISTS (SELECT 1 FROM search_entity_class)" \
  ./embed_missing.sh search_corpus doc "$WORKERS" "src_table,row_key" || fail "векторы корпуса (бизнес)"

echo "== 3-alt. векторы остальным строкам корпуса"

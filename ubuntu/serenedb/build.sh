...
# и его данные не обновлялись никогда. Имя берётся из DSN — то есть из того же источника,
# что и сама база, без отдельной настройки. [найдено 28.07 при тесте на второй базе]
LOCK_TAG=$(printf '%s' "$DSN" | sed -n 's/.*dbname=\([A-Za-z0-9_]*\).*/\1/p'); LOCK_TAG=${LOCK_TAG:-default}

...
#
# Это то, что запускает таймер вместо `serene_search_build.py`. Питон в сборке остался
# ровно в одном шаге — выбор денежной колонки моделью (`pick_money_col.py`), потому что
# вызова языковой модели у движка нет. Пункт 20 `TARGET.md` разрешает это явно.
#
...
#   3. модель называет денежную колонку — с кэшем, поэтому обычно ноль обращений;
#   4. слияние в боевой корпус по отпечатку + публикация индекса;
#   5. векторы досчитываются только новым строкам;
#   6. резолвер — тем же способом: значения по ключу, векторы только новым;
#   7. секреты удаляются. 🔴 Это обязательный шаг: «временный» секрет SereneDB
...
echo "== 2. денежная колонка (модель, с кэшем)"
python3 pick_money_col.py || echo "предупреждение: шаг денежной колонки не отработал" >&2

echo "== 3. слияние в боевой корпус и публикация индекса"
psql "$DSN" -q -f corpus_merge.sql || fail "слияние корпуса"

echo "== 4. векторы новым строкам корпуса"

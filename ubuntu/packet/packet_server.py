...
def _reload_bases_if_changed() -> None:
    ...
    Зовётся из _auth_base — перед проверкой доступа КАЖДОГО запроса: слоты
    добавляются на живом приёмнике (onboard), config-builder (packet_config.py)
    переписывает запись базы атомарно — без перечитывания новая база получала
    бы 401 до перезапуска приёмника. Битый или полупустой файл игнорируется —
    остаётся прежняя версия (fail-stale, а не fail-open и не падение
    работающего приёмника)."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _auth_base(self, base_id: str) -> bool:
        # Перечитывание по mtime обязано идти ДО проверки: иначе база, добавленная
        # после старта приёмника, получала 401 на PUT manifest, пока кто-нибудь не
        # дёрнет /agent/config (живой случай 10.08: слот novaya-baza-5 на юните,
        # приёмник стартован 08.08 — «токен отклонён» при верном токене).

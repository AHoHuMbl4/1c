...
# Приёмка панелей — отдельным инструментом (grafana/check-panels.py):
# он же зовётся руками после правки дашбордов.
PYEOF
...
# ловушка 11]. Пароль идёт через окружение, не через аргументы.
GRAFANA_ADMIN_PASSWORD="$GRAFANA_ADMIN_PASSWORD" \
    python3 "$SCRIPT_DIR/grafana/check-panels.py" --url http://127.0.0.1:3001

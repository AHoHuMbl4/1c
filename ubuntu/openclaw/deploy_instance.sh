...
# `/opt/openclaw-mcp` и перезапуск юнита моста идут по жёстким путям — и
...
# Подтверждение «сделано» при сухом прогоне не печатается: строка «мост перезапущен»
...
if [ -d "$MCPDIR" ]; then
  ...
  # что обслуживает ЭТОТ экземпляр бота. 🔴 УМОЛЧАНИЯ-ИМЕНИ БОЛЬШЕ НЕТ (07.08): с 02.08
  # боевые мосты — шаблонные `1c-mcp-ask@<база>`, а прежнее умолчание `1c-mcp-ask` —
  # осиротевший юнит без MCP_PORT: его рестарт 04.08 поставил его в петлю падений на
  # два дня (36 980 рестартов, bind :6016 занят боевым). Поэтому без явного
  # OPENCLAW_ASK_UNIT перезапускаются ВСЕ активные шаблонные мосты: код у них один,
  # рестарт безопасен, а «не тот мост» — это молчаливая старина кода, ради которой
  # скрипт и существует.
  if [ -n "${OPENCLAW_ASK_UNIT:-}" ]; then
    ASK_UNITS="$OPENCLAW_ASK_UNIT"
  else
    ASK_UNITS=$(systemctl list-units --type=service --state=active --no-legend \
                '1c-mcp-ask@*.service' 2>/dev/null | awk '{print $1}')
  fi
  for ASK_UNIT in $ASK_UNITS; do
    run systemctl restart "$ASK_UNIT" 2>/dev/null && ok "выкатка: $ASK_UNIT перезапущен"
  done
  [ -n "$ASK_UNITS" ] || echo "выкатка: 🔴 активных юнитов 1c-mcp-ask@* нет — мост не перезапущен" >&2

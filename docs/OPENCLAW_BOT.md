...
состояние**, пока владелец не решит: (а) вернуть прежнюю и вычищать её по одной правке с
замером на каждую, либо (б) оставить новую и добирать потерянное.

Значит `diff` репозитория с рабочей папкой сейчас РАСХОДИТСЯ, и это не забытая выкатка.
Развилка записана в [`memory_bank/activeContext.md`](../memory_bank/activeContext.md),
раздел «Развилка, которую нужно решить владельцу».

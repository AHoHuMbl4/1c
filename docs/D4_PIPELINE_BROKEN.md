...
Выкат на okna (27.08): `build.sh` + `corpus_init.sql` + `corpus_precheck.sql`
→ `/opt/1c-mcp-reports` (бэкапы `.bak-d4-*`). Таймер остановлен на время
пробы, затем снова `active`.

| Шаг | Результат |
|---|---|
| `== 0. объекты поиска` | **OK** — `объекты поиска на месте … solr_syn_dict_ok = 1` |
| `corpus_precheck` | **OK** — `проверки до такта пройдены`, корпус 1 230 156 |
| `== 1. корпус` | **FAIL** — `$metadata пусты или неполны: сущностей 0` |
| `systemctl is-active` после прогона | `failed` (уже **не** «развёртывание объектов») |

Хвост (27.08 08:57:27):

```text
объекты поиска на месте | 11 | 1 | 1
проверки до такта пройдены | 1230156 | 69621
== 1. корпус: … из /var/lib/serenedb/packet-meta/okna-1
ERROR: $metadata пусты или неполны: сущностей 0, с ключом 0
СБОРКА ПРЕРВАНА: сборка корпуса
```

Свежесть: `build_ts=1787550031` (≈24.08), `mart_changed_ts=1787817782`
(новее — синк/packet apply шли, пока такт умирал на init). Предупреждение
бота про «N мин назад» **не** снято: корпус не пересобран.

### Следующий блокер (не Д4)

В `packet-meta/okna-1/` нет файла `$metadata` — только `skipped.json`.
Все 74 пакета inbox `kind=delta`, `metadata.included=false` (ни одного
`kind=meta`). До 24.08 05:40 такты проходили за счёт **пропуска** пересборки
корпуса; после трёх суток синка без сборки `mart_changed > build_ts` → нужна
пересборка → нужен снимок. Чинить агентом Windows (`kind=meta` /
`metadata.included=true`), не подстановкой psql-переменных.

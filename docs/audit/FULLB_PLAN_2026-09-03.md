...
- 0f. Колонка `mode` в `tmp3_build` (единый CTE: членство + маркеры + мост —
  ...
  **L4(б) при ЛЮБОМ flip; при flip=0 дублируется сторожем A/L0** (G2: без
  ...
  (P10). **Interim до flip=1 (правка 10.09, R6; прежняя формула F3 «interim до
  этапа 2» снята — мост есть, риск инвертирован):** при `:partial_rebuild=0`
  CTAS пишет `mode='full'` ВСЕМ (сторож A иначе STOPит честный partial —
  зелёный прод ломается порядком); честный расчёт по мосту — только при
  flip=1. Заглушек-режимов не заводим. **Исполнение 0f (G3):** правит
  corpus_build.sql — общий файл со Speed-II-2 ⇒ 0f идёт ОДНИМ выкатом с
  завершением Speed-II-2 (не параллельно ему); сторож A (читает колонку)
  вводится только после 0f. До того 0a верен: прод-механизмы этапа 0 —
  только 0c/0d. **Исполнено 10.09** (Speed-II-2+0f+A одним пакетом;
  rebuild_mode-отчёт стартует с 0f в build — M8 потребляет те же ярлыки).

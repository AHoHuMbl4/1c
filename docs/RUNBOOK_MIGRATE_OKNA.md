...
| **Проход 1** rsync `store.db` old→new (живьём, ~103 МБ/с) | 🟢 **[замер 22.08]** |
| Tar-поток конфигов/юнитов/`/opt`/`/home/undebot` (без venv, транзиента) | 🟢 в работе / частично |
| Установка бинаря 26.08.1, правка `serened.conf`, пересборка venv | ⏳ ждёт |
| Пересчёт эмбеддингов 4B на новой коробке | ⏳ ждёт |
...
**[решение 22.08]** Цель сразу на **26.08.1** (совместимость store.db доказана Ф1 — `docs/UPGRADE_F1_REPORT.md` §3).
...
**[замер 22.08]** Скорость ~**103 МБ/с**. Выполнен с источника или с dev через ключ миграции.
...
# ожидание: version содержит 26.08.1; count ≈ 1 230 000 (точное — с источника)
```
...
| из контейнеров okna/klient1 | `http://10.10.10.1:8000/v1/embeddings` |
...
EMBED_RESET=1 EMBED_TARGETS="labels resolver card corpus" ./embed_all.sh
```
...
| 4 | rsync `store.db` (+ wal) **или** firstbuild с нуля | `<SRC_HOST>`, `<MIGRATE_KEY>` |
...
| S3 (мелкие артефакты) | `s3.twcstorage.ru`, bucket `klient1-migration` (~365 МБ конфигов) |

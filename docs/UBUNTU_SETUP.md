...
| Мост (бот+API) | pip venv `/opt/bridge-venv` (aiogram 3, FastAPI, asyncpg, pymorphy3…) | `tg-bridge` (**disabled** — Telegram забрал OpenClaw, см. `OPENCLAW_BOT.md`), `api` (0.0.0.0:8090, кормит `ask_1c`) |
...
7. Стыковка с 1С-контуром: холодный ETL (`ubuntu/1c-etl/`, `RUNBOOK_DEPLOY.md` §8) пишет в KB-репо.

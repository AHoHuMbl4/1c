...
| ~~Ollama + bge-m3~~ | **НЕ СТАВИМ — решение владельца 2026-07-22 («Ollama-standby плохо себя показал, отметаем полностью»).** Шаг 2/6 install.sh пропускаем; `Wants=ollama.service` в open-webui.service — мягкая зависимость, старт без неё работает; в `.env` сразу ставим эмбеддер DashScope (не дефолтный `RAG_EMBEDDING_ENGINE=ollama`), чтобы окно «до bootstrap» не зависело от ollama. Принятый риск: fallback-эмбеддера нет — при недоступности DashScope индексация и поиск стоят | — |
...
4. ~~Ollama~~ — решено владельцем: не ставим (см. §2).

...
## 6. Живая проба оркестратора (27.08, стейджинг okna :8092)

Кандидат с правками К5 поднят отдельным процессом на okna
(`serene_ask_cand.py`, md5 `4ec52861e6e0ecb5dfa437c3a4cc9065`), боевой `:8091`
не трогали.

| набор | до К5 | после К5 |
|---|---|---|
| `AB_PROBE=okna` (8 вопросов) | 8/8, средняя 18,52 с | **8/8, средняя 20,70 с** |

Регресса нет — это и было условием коммита `serene_ask.py`. Отметку
`.claude/.probe-okna-last-run` поставил сам прогон (0err/8).

Отличие в одном пункте: «сколько позиций у нас в прайсе?» вернулся как
`kind=figures` вместо `answer` — обе формы набор засчитывает, число верное.

Замер неоднозначных (18 вопросов, база 5/18) на этом кандидате ещё не снят —
это следующий шаг; результат идёт в `ACCEPTANCE_AMBIGUOUS.md` без подгонки,
каким бы он ни был.

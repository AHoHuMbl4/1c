...
🔴 **[замер 10.09] Железо окна сменилось: gpu-erw.timpul.pro (ssh :2202), 40 vCPU /
125 GiB** (замер 17.08 выше — прежнее okna 167.233.249.110, 4 vCPU / 7,6 GiB; его
`cpu_threads=4` на новом железе душит такт). При `cpu_threads=4` предчек
`corpus_precheck.sql:94` падал: «пул исполнителей = 3, а досчёт просит минимум 4» —
тот же корень, что «движок жуёт на ~1 ядре» при пустых остальных. С 10.09:

```conf
# /etc/serenedb/serened.conf — okna (gpu-erw), 40 vCPU / 125 GiB
--cpu_threads=34      # предчек досчёта: пул ≥ BUILD_THREAD_MIN; RAM запас 101 GiB
--io_threads=8
```

Рестарт `serenedb` обязателен (настройки читаются при старте); вектора и NULL-счётчик
пережили рестарт без изменений (замер 10.09: NULL 446732 до/после).
...
### 10.9-ter Откат пакета Speed-II-2+0f+сторож A (10.09)

Пакет = ТРОЙКА файлов атомарно: `corpus_build.sql` + `corpus_merge.sql` +
`corpus_init.sql` (md5 пары/тройки — полуоткат merge = тихая потеря сторожа A).
Протокол отката:

1. Вернуть прежние версии всех трёх файлов (scp из git-истории) — только ПАРОЙ/ТРОЙКОЙ.
2. `DROP TABLE IF EXISTS search_cls_numhint;` — остаток кэша в базе после
   возврата кода невидим старому коду, но опасен при повторном выкате с иной
   сигнатурой.
3. Один полный такт numhint: следующий проход с `NOT tmp3_inc` (первый такт
   после смены SQL-хэша) пересчитает классификацию целиком — прежним
   сессионным путём.
4. `rebuild_mode:*` / `build_phase:*` строки в `search_quality` станут мёртвыми —
   стереть руками: `DELETE FROM search_quality WHERE k LIKE 'rebuild_mode:%' OR k LIKE 'build_phase%';`
...
шагами (поле `<мс>` — первое **голое целое** после слоя, от начала ответа сервиса;
`in=N` / `строк=N` — штуки в статусе, без суффикса `ms`; между слоями смотреть `+Nms`

...
### 7.16. K9 фаза 5: event-дуэль → A/B/C, подпись оси (28.08)

| механизм | где | что |
|---|---|---|
| event-дуэль | `event_duel_applies`, `try_event_code_entity_pick` (~2587, ~2671) | ≥2 осевых движения при event+count → `picked` в fork, не clarify |
| DISTINCT в fork | `_event_distinct_fork_rows`, `fork_detector_scan` (~2605, ~4610) | COUNT(DISTINCT ось) вместо row count для классов A/B/C |
| исходы | `resolve_fork_outcome` / `fork_outcome_a/b/c` (~6810, ~14575) | равные числа→A; разные+подписи→B; без подписей→C; defer снят |
| подпись оси | `render_atom_pair` (~8216), `_passport_axis_col` (~10104) | distinct-ответ: «N · метка оси» из словаря базы |

**Замки офлайн (сессия 28.08 ф5):**

| замок | до | после |
|---|---|---|
| test_action_class | 23/0 | **27/0** (+event A/B/C, axis render, fork rows) |
| test_sales_rank_canon | 51/0 | **51/0** |
| test_rank_leader_path | 30/0 | **30/0** |
| test_entity_form | 47/0 | **47/0** |
| test_k6_rank_v2 offline | 10/0 | **10/0** |
| test_compose | 92/0 | **92/0** |
| bench v2_lead / v1-регресс | 20/23, 0 | **оркестратор** |

**Таблица ожиданий живых проб (кандидат :8096):**

| вопрос | ожидание ф5 |
|---|---|
| «сколько клиентов реально покупают» | **141** (A или B), не clarify двух движений |
| «сколько покупателей было за этот месяц» | **N · Контрагенты** (подпись оси), не «assumed: N» |
| «наторговали в прошедшее воскресенье?» | answer (регресс снят ф4) |
...
## 11. Закрытие двойки (29.08, live)

С2 live: «сколько клиентов реально покупали за последний год» → **141 · Контрагенты** (=эталон; после восстановления словаря §3.99, стабильно ×3). Без периода — честный clarify периода. С4 live: «сколько позиций совсем не продаётся…» → **clarify** (гвард исхода C, `docs/C4_K6_BREAKDOWN.md §8`). K6 live 0/2 → **2/2**: двойка закрыта, дорога К8 открыта.

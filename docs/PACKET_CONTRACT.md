...
- **Применение:** все чанки на месте → verify (age + sha256 + zstd) синхронно в `status` →
  состояние `verified`; перевод `verified → applied` выполняет apply-компонент **одной
  транзакцией на пакет** (К7). Сбой verify/apply → `quarantined` + `error`, витрина не
  тронута. Агент по `quarantined` не повторяет тот же пакет — ждёт решения (новый `seq`).

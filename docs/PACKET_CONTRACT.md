...
**Пилотный режим (решение владельца 06.08, «быстро, пилот»):** слой age может быть
снят на комплекте без `recipient_pubkey` — тогда файлы идут открытыми:
`manifest.json`, `<chunk-name>.csv.zst` (служебные — `.zst`). Приёмник определяет
режим **по магии файла** (`age-encryption.org/` — age; байты `28 B5 2F FD` — zstd;
иначе 409 `bad_format`), оба режима принимаются одновременно; снятие age не меняет
ни манифест, ни имена чанков, ни протокол. Возврат age — появлением pubkey в
комплекте, без смены `manifest_version`. Цена режима записана честно: в открытом
виде данные видны на релее после TLS-терминации и в карантине приёмника на диске;
канал держат TLS (Windows→релей) + SSH-туннель (релей→Ubuntu) + mTLS.
...
- Шифрование тела пакета: **age v1** (https://age-encryption.org), получатель **X25519**,
  ...
  ⚠ **В пилоте слой age снят** (решение владельца 06.08, см. §2): тело идёт открытым
  zstd внутри TLS+SSH+mTLS; возврат — без смены версии.

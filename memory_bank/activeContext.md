...
## Состояние (29.08, corpus_merge сторож — в работе)

- **corpus_merge**: правило `entity_key_form_changed` — per-entity 100 % уход
  старых ключей при непустой новой не меньше → запись в search_quality,
  исключение из порога 10 %; частичный/shrink → STOP. Замок **17/0**.
- **okna такт merge STOP** (29.08 11:11): сторож **:151** «частичная потеря»
  у 3 регистров (23–29 строк из ~75–189k каждый) — **не** смена формы ключа
  (там 100 % уход). `entity_key_form_changed` **0**, корпус **1 230 156** цел.
  Следующий шаг: диагноз 23 unmatched по каждому регистру (orphan vs дефект
  анти-соединения), **сторож не ослаблять**.
- **Детерминированность / ask :8091** — без изменений (HEAD 2c18fc6).

...
**Дальше:** график картинкой (PNG→Telegram) + read-only роль; Этап 3 семантический резолвер; штатный
инкрементальный синк ETL→SereneDB; backup/restore (тест+фаундеры). RAG braine — второй инструмент.

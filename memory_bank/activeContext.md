...
## Состояние (29.08, corpus_merge — deleted_delta ветка)

- **corpus_merge**: четыре ветки сторожа — `entity_key_form_changed` (100 % уход),
  `entity_key_collapse` (частичный уход + рост + Recorder жив),
  `entity_deleted_delta` (частичный уход + мёртвый Recorder + документ удалён),
  иначе STOP / «дефект транспорта». Замок **37/0**.
- **okna такт merge STOP** (ожидаемо): `f7f10236` — документ **жив** (19 строк
  `document_реализациятмц`), движений в трёх регистрах **0** → дефект транспорта,
  не обходить. `de0ef63e`/`f4e8e0fa` — документ удалён (deleted_delta, 16 строк).
  Корпус **1 230 156**, tmp3 **1 561 704**. Такт красный до починки пакета/recorder.

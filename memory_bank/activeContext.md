...
🔴 **rank axis + stock no_data — код в git `[18.08]`**: билет оси не схлопывает
rank в скаляр; «товар» → номенклатура + Количество; остаток без balance_registers
→ no_data честно. Оффлайн **13/13** `test_rank_axis_anchor.py` + F-замки

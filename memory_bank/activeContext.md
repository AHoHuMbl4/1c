...
**Нативный путь найден (docs движка `docs/cli/mcp.md`):** OpenClaw — нативный MCP-клиент (`openclaw mcp add`, транспорт streamable-http/stdio) → инструменты MCP-сервера доступны боту в messaging-профиле. План: тонкий MCP-сервер-обёртка над braine `/ask` → `openclaw mcp add` → персона направляет. ⚠ Обёртка = наш код → по правилу №1 подтвердить у владельца перед реализацией. Инвариант: OpenClaw переформулирует ТОЛЬКО факты braine, не досочиняя.

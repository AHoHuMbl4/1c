...
🔴 **золотой набор okna собран ИЗ ДАННЫХ — код в git `[19.08]`**:
`ubuntu/serenedb/ab_gold_build.py` строит `ab-gold-okna.tsv` (**15** вопросов,
9 классов) по метаданным корпуса, без имён конкретной базы; чисел в наборе нет —
эталон считает SQL при прогоне. Снимок «вопрос → ответ» для чтения —
`docs/AB_GOLD_OKNA.md`. `ab_scorer.py` научен третьему полю (`CLS=`, `PICK=`) и
кликам по развилке билетом `decision_id`; режимы `kind`/`fork`/`pivot`. Оффлайн
`test_ab_gold.py` **30/30**. md5: build **`9cd5d351`**, scorer **`0b63a20c`**,
tsv **`2c92c4ba`**, ответы **`2d2d8cca`**. **Приёмку и правки кода под эти вопросы
делает другая сессия** (решение владельца 19.08) — здесь только набор и ответы.
**Дальше:** гейт ДО выката — теневой `:8092` на okna с кандидатным кодом и RO-ролью,
отметка с md5 кандидата (разбор — в отчёте сессии и CHANGELOG 19.08).


🔴 **дашборды okna — вход жив, панели чинить выкатом `[19.08]`**: владелец
...
выгрузка `grafana/export-dashboards.py`. **Выкат сделан `[19.08]`**: на фронте
`json_data` = `{"database":"postgres","postgresVersion":1500,"sslmode":"disable"}`,
probe `PostgreSQL 18.3 (SereneDB 26.07.3)`, живой дашборд не перетёрт. Заведён
**шаг 9 — приёмка панелей** (`rawSql` каждой панели через `/api/ds/query`,
пустая роняет установку): probe при этой поломке проходил. **Дальше:** кнопка
«добавить в дашборд» (OWUI Action → API панелей).

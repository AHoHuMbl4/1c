...
🔴 **догон корпуса okna — замок в git `[18.08]`**: синк packet больше не
затирает `search_changed_sources` до rebuild. Журнал 18:41/18:51: «таблицы: 0»
→ «пересобирали 0 из 351». Код: `changed_sources_sql` + `tmp_changed_keep` +
`tmp3_lag`. Оффлайн **17/17**. Таймер пайплайна остановлен для догона.
Отчёт: `docs/CORPUS_VITRINE_OKNA_2026-08-18.md` §5.

...
выгрузка `grafana/export-dashboards.py`. **Дальше:** выкат на фронт (rsync
`ubuntu/open-webui/` → `/opt/1c-open-webui/` + прогон установщика) — в этой
сессии закрыт классификатором среды; приёмка — открытый дашборд с данными,
затем кнопка «добавить в дашборд» (OWUI Action → API панелей).

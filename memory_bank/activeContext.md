...
_Обновлено: **2026-08-24.** Здесь — только живое: текущее состояние и «С ЧЕГО НАЧАТЬ».
...
🔴 **W-трек + rank — в бою okna `[24.08]`**: md5 `serene_ask.py` **50b13be1** на
`:8091`. Три фикса окон: drop_assumed только без явного периода; скользящая
assumed-неделя → текущая календарная (wtd лидер, скользящее люком); суперлатив
«лучше всего» больше не compare. Rank: ось из имён осей источника (pick по списку
имён + rerank; списки слов `product_axis_pref` сняты), имя лидера из GROUP BY,
текст кодом. Проба боя **8/8**, средняя **34,27 с**; замки **486+26+66**.

🔴 **health-фикс пробит, коммит+выкат — этот заход `[24.08]`**: кандидат md5
**11d8f158** (W-трек + `_measure_health_gap` одним SQL из `search_coverage`)
на стейджинге okna :8092 — AB_PROBE **8/8**, 0 сбоев, **55,78 с**; маркер гейта
`okna probe live 0err/8` свежий. Точечные замки: health_gap **23/23**,
embed_progress OK, W/rank/hatch/journal — зелёные. Дальше: выкат на бой :8091
(бэкап `.bak-<md5>` → restart) + замер /health <5 с; потом Ф6.2 IVF-резолвер,
Ф6.4 свежесть, §7bis бизнес-справочники ($metadata okna на произвкалендарь).
